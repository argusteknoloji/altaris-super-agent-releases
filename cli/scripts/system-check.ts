...
function currentBaseUrl(): string {
  if (isTruthy(process.env.ALTARIS_USE_GEMINI)) {
    return process.env.GEMINI_BASE_URL ?? GEMINI_DEFAULT_BASE_URL
  }
  if (isTruthy(process.env.ALTARIS_USE_MISTRAL)) {
    return process.env.MISTRAL_BASE_URL ?? MISTRAL_DEFAULT_BASE_URL
  }
  if (isTruthy(process.env.ALTARIS_USE_GITHUB)) {
...
function checkOpenAIEnv(): CheckResult[] {
  const results: CheckResult[] = []
  const useGemini = isTruthy(process.env.ALTARIS_USE_GEMINI)
  const useGithub = isTruthy(process.env.ALTARIS_USE_GITHUB)
  const useMistral = isTruthy(process.env.ALTARIS_USE_MISTRAL)
  const useOpenAI = isTruthy(process.env.ALTARIS_USE_OPENAI)
  ...
  if (!useOpenAI) {
    results.push(pass('Provider mode', 'Anthropic login flow enabled (ALTARIS_USE_OPENAI is off).'))
    return results
  }
...
function checkOllamaProcessorMode(): CheckResult {
  if (
    !isTruthy(process.env.ALTARIS_USE_OPENAI) ||
    isTruthy(process.env.ALTARIS_USE_GEMINI) ||
    isTruthy(process.env.ALTARIS_USE_GITHUB) ||
    isTruthy(process.env.ALTARIS_USE_MISTRAL)
  ) {
...
function serializeSafeEnvSummary(): Record<string, string | boolean> {
  ...
  return {
    ALTARIS_USE_OPENAI: isTruthy(process.env.ALTARIS_USE_OPENAI),
