...
function parseLaunchOptions(argv: string[]): LaunchOptions {
  ...
  let fast = false
  let goal = normalizeRecommendationGoal(process.env.ALTARIS_PROFILE_GOAL)
...
function applyFastFlags(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  env.ALTARIS_SIMPLE ??= '1'
  env.ALTARIS_DISABLE_THINKING ??= '1'
  env.DISABLE_INTERLEAVED_THINKING ??= '1'
  env.DISABLE_AUTO_COMPACT ??= '1'
  env.ALTARIS_DISABLE_AUTO_MEMORY ??= '1'
  env.ALTARIS_DISABLE_BACKGROUND_TASKS ??= '1'
  return env
}
