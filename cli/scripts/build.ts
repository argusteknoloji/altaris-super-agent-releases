/**
 * Altaris build script — bundles the TypeScript source into a single
...
const featureFlags: Record<string, boolean> = {
  ...
  VOICE_MODE: false,              // Push-to-talk STT via altaris.ai OAuth endpoint
...
const result = await Bun.build({
  ...
  define: {
    ...
    // version separately in Open Altaris branding.
    'MACRO.VERSION': JSON.stringify('99.0.0'),
    'MACRO.DISPLAY_VERSION': JSON.stringify(version),
    'MACRO.BUILD_TIME': JSON.stringify(new Date().toISOString()),
    'MACRO.ISSUES_EXPLAINER':
      JSON.stringify('report the issue at https://github.com/anthropics/altaris/issues'),
    'MACRO.PACKAGE_URL': JSON.stringify('@gitlawb/altaris'),
...
if (!result.success) {
  ...
  console.log(`✓ Built altaris v${version} → dist/cli.mjs`)
}
