...
import { accumulateUsage, updateUsage } from 'src/services/api/altaris.js'
...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // a memory directory and needs Altaris to know how to use it (which
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.ALTARIS_EAGER_FLUSH) ||
          isEnvTruthy(process.env.ALTARIS_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via ALTARIS_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or ALTARIS_PLUGIN_SEED_DIR before this runs;
    ...
    for await (const message of query({
      ...
      if (
        ...
        if (persistSession) {
          // Fire-and-forget for assistant messages. altaris.ts yields one
      ...
      switch (message.type) {
        ...
        case 'stream_event':
          ...
          if (message.event.type === 'message_delta') {
            ...
            // real value only arrives here (see altaris.ts message_delta
...
/**
 * Sends a single prompt to the Model Provider API and returns the response.
 * Assumes that altaris is being used non-interactively -- will not
