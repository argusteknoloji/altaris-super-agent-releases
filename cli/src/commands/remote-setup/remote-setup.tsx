...
function Web({
    ...
    void checkLoginState().then(async result => {
      switch (result.status) {
        case 'not_signed_in':
          ...
          onDone('Not signed in to Altaris. Run /login first.');
          return;
  ...
  if (step.name === 'uploading') {
    return <LoadingState message="Connecting GitHub to Altaris…" />;
  }
  const token = step.token;
  return <Dialog title="Connect Altaris on the web to GitHub?" onCancel={handleCancel} hideInputGuide>
      <Box flexDirection="column">
        <Text>
          Altaris on the web requires connecting to your GitHub account to clone
