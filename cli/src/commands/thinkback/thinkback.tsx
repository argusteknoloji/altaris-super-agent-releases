...
const INTERNAL_MARKETPLACE_NAME = 'altaris-marketplace';
const INTERNAL_MARKETPLACE_REPO = 'anthropics/altaris-marketplace';
...
export async function playAnimation(skillDir: string): Promise<{
  ...
  // silently return success. Using readFile (not access) per ALTARIS.md.
  //
...
function ThinkbackMenu(t0) {
  ...
  let t4;
  if ($[8] !== hasGenerated) {
    t4 = !hasGenerated && <Box flexDirection="column"><Text>Relive your year of coding with Altaris.</Text><Text dimColor={true}>{"We'll create a personalized ASCII animation celebrating your journey."}</Text></Box>;
    $[8] = hasGenerated;
    $[9] = t4;
  } else {
    t4 = $[9];
  ...
  if ($[16] !== handleCancel || $[17] !== t6) {
    t7 = <Dialog title="Think Back on 2025 with Altaris" subtitle="Generate your 2025 Altaris Think Back (takes a few minutes to run)" onCancel={handleCancel} color="altaris">{t6}</Dialog>;
    $[16] = handleCancel;
    $[17] = t6;
    $[18] = t7;
  } else {
...
const EDIT_PROMPT = 'Use the Skill tool to invoke the "thinkback" skill with mode=edit to modify my existing Altaris year in review animation. Ask me what I want to change. When the animation is ready, tell the user to run /think-back again to play it.';
const FIX_PROMPT = 'Use the Skill tool to invoke the "thinkback" skill with mode=fix to fix validation or rendering errors in my existing Altaris year in review animation. Run the validator, identify errors, and fix them. When the animation is ready, tell the user to run /think-back again to play it.';
const REGENERATE_PROMPT = 'Use the Skill tool to invoke the "thinkback" skill with mode=regenerate to create a completely new Altaris year in review animation from scratch. Delete the existing animation and start fresh. When the animation is ready, tell the user to run /think-back again to play it.';
