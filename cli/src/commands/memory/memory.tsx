...
import { clearMemoryFileCaches, getMemoryFiles } from '../../utils/altarismd.js';
...
function MemoryCommand({
  ...
  const handleSelectMemoryFile = async (memoryPath: string) => {
    try {
      // Create altaris directory if it doesn't exist (idempotent with recursive)
  ...
  return <Dialog title="Memory" onCancel={handleCancel} color="remember">
      <Box flexDirection="column">
        ...
        <Box marginTop={1}>
          <Text dimColor>
            Learn more: <Link url="https://code.altaris.com/docs/en/memory" />
          </Text>
        </Box>
      </Box>
    </Dialog>;
