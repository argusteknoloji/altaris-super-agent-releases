...
export function registerMcpXaaIdpCommand(mcp: Command): void {
  ...
  xaaIdp
    ...
    .requiredOption('--client-id <id>', "Altaris's client_id at the IdP")
    .option(
      '--client-secret',
  ...
  xaaIdp
    ...
    .action(async options => {
      ...
      if (!idp) {
        return cliError(
          "Error: no XAA IdP connection. Run 'altaris mcp xaa setup' first.",
        )
      }
  ...
  xaaIdp
    ...
    .action(() => {
      ...
      process.stdout.write(
        `Logged in:     ${hasIdToken ? 'yes (id_token cached)' : "no — run 'altaris mcp xaa login'"}\n`,
      )
      cliOk()
    })
