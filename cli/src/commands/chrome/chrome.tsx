...
import { CLAUDE_IN_CHROME_MCP_SERVER_NAME, openInChrome } from '../../utils/altarisInChrome/common.js';
import { isChromeExtensionInstalled } from '../../utils/altarisInChrome/setup.js';
import { getGlobalConfig, saveGlobalConfig } from '../../utils/config.js';
import { env } from '../../utils/env.js';
import { isRunningOnHomespace } from '../../utils/envUtils.js';
const CHROME_EXTENSION_URL = 'https://altaris.ai/chrome';
...
function ClaudeInChromeMenu(t0) {
        ...
        case "toggle-default":
          {
            ...
            saveGlobalConfig(current => ({
              ...current,
              altarisInChromeDefaultEnabled: newValue
            }));
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>Altaris Browser Bridge works with the Chrome extension to let you control your browser directly from Altaris. Navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">Altaris Browser Bridge is not supported in WSL at this time.</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  if ($[23] !== isClaudeAISubscriber) {
    t8 = true && !isClaudeAISubscriber && <Text color="error">Altaris Browser Bridge requires a altaris.ai subscription.</Text>;
  ...
  if ($[25] !== handleAction || $[26] !== isConnected || $[27] !== isDisabled || $[28] !== isExtensionInstalled || $[29] !== options || $[30] !== selectKey || $[31] !== showInstallHint) {
    t9 = !isDisabled && <>{!isHomespace && <Box flexDirection="column"><Text>Status:{" "}{isConnected ? <Text color="success">Enabled</Text> : <Text color="inactive">Disabled</Text>}</Text><Text>Extension:{" "}{isExtensionInstalled ? <Text color="success">Installed</Text> : <Text color="warning">Not detected</Text>}</Text></Box>}<Select key={selectKey} options={options} onChange={handleAction} hideIndexes={true} />{showInstallHint && <Text color="warning">Once installed, select {"\"Reconnect extension\""} to connect.</Text>}<Text><Text dimColor={true}>Usage: </Text><Text>altaris --chrome</Text><Text dimColor={true}> or </Text><Text>altaris --no-chrome</Text></Text><Text dimColor={true}>Site-level permissions are inherited from the Chrome extension. Manage permissions in the Chrome extension settings to control which sites Altaris can browse, click, and type on.</Text></>;
    $[25] = handleAction;
    $[26] = isConnected;
    $[27] = isDisabled;
  ...
  if ($[33] === Symbol.for("react.memo_cache_sentinel")) {
    t10 = <Text dimColor={true}>Learn more: https://code.altaris.com/docs/en/chrome</Text>;
    $[33] = t10;
  } else {
    t10 = $[33];
  }
  ...
  let t12;
  if ($[38] !== t11 || $[39] !== t5) {
    t12 = <Dialog title="Altaris Browser Bridge (Beta)" onCancel={t5} color="chromeYellow">{t11}</Dialog>;
    $[38] = t11;
    $[39] = t5;
    $[40] = t12;
  } else {
...
export const call = async function (onDone: (result?: string) => void): Promise<React.ReactNode> {
  ...
  return <ClaudeInChromeMenu onDone={onDone} isExtensionInstalled={isExtensionInstalled} configEnabled={config.altarisInChromeDefaultEnabled} isClaudeAISubscriber={isSubscriber} isWSL={isWSL} />;
};
