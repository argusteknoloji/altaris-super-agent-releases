...
import { useClaudeAiLimits } from '../../services/altarisAiLimitsHook.js';
...
function RateLimitOptionsMenu(t0) {
  ...
  bb0: {
    let actionOptions;
    if ($[2] !== altarisAiLimits.overageDisabledReason || $[3] !== altarisAiLimits.overageStatus) {
      actionOptions = [];
      if (extraUsage.isEnabled()) {
        const hasBillingAccess = hasClaudeAiBillingAccess();
        const needsToRequestFromAdmin = isTeamOrEnterprise && !hasBillingAccess;
        const isOrgSpendCapDepleted = altarisAiLimits.overageDisabledReason === "out_of_credits" || altarisAiLimits.overageDisabledReason === "org_level_disabled_until" || altarisAiLimits.overageDisabledReason === "org_service_zero_credit_limit";
        if (needsToRequestFromAdmin && isOrgSpendCapDepleted) {} else {
          const isOverageState = altarisAiLimits.overageStatus === "rejected" || altarisAiLimits.overageStatus === "allowed_warning";
          let label;
      ...
      $[2] = altarisAiLimits.overageDisabledReason;
      $[3] = altarisAiLimits.overageStatus;
      $[4] = actionOptions;
    } else {
      actionOptions = $[4];
    }
