...
export function ApiKeyStep(t0) {
  ...
  if ($[27] !== existingApiKey || $[28] !== selectedOption || $[29] !== theme) {
    t11 = existingApiKey && <Box marginBottom={1}><Text>{selectedOption === "existing" ? color("success", theme)("> ") : "  "}Use your existing Altaris API key</Text></Box>;
    $[27] = existingApiKey;
    $[28] = selectedOption;
    $[29] = theme;
    $[30] = t11;
  ...
  if ($[31] !== onCreateOAuthToken || $[32] !== selectedOption || $[33] !== theme) {
    t12 = onCreateOAuthToken && <Box marginBottom={1}><Text>{selectedOption === "oauth" ? color("success", theme)("> ") : "  "}Create a long-lived token with your Altaris subscription</Text></Box>;
  ...
  if ($[40] !== apiKeyOrOAuthToken || $[41] !== cursorOffset || $[42] !== onApiKeyChange || $[43] !== onSubmit || $[44] !== selectedOption || $[45] !== terminalSize) {
    t15 = selectedOption === "new" && <TextInput value={apiKeyOrOAuthToken} onChange={onApiKeyChange} onSubmit={onSubmit} onPaste={onApiKeyChange} focus={true} placeholder={"sk-ant\u2026 (Create a new key at https://platform.altaris.com/settings/keys)"} mask="*" columns={terminalSize.columns} cursorOffset={cursorOffset} onChangeCursorOffset={setCursorOffset} showCursor={true} />;
