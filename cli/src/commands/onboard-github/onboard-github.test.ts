...
describe('onboarding auth precedence cleanup', () => {
  test('clears preexisting OpenAI auth when switching to GitHub', () => {
    const env: NodeJS.ProcessEnv = {
      ...
      ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID: 'profile_old',
    }

    applyGithubOnboardingProcessEnv('github:copilot', env)

    expect(env.ALTARIS_USE_GITHUB).toBe('1')
    ...
    expect(env.ALTARIS_USE_OPENAI).toBeUndefined()
    expect(env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED).toBeUndefined()
    expect(env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID).toBeUndefined()

    const settingsEnv = buildGithubOnboardingSettingsEnv('github:copilot')
    expect(settingsEnv.ALTARIS_USE_GITHUB).toBe('1')
