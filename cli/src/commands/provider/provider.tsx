...
export function buildCurrentProviderSummary(options?: {
  ...
  if (isEnvTruthy(processEnv.ALTARIS_USE_GEMINI)) {
    return {
  ...
  if (isEnvTruthy(processEnv.ALTARIS_USE_MISTRAL)) {
    return {
  ...
  if (isEnvTruthy(processEnv.ALTARIS_USE_OPENAI)) {
...
export function buildProfileSaveMessage(
  ...
  if (options?.activatedInSession) {
    lines.push('Altaris switched to it for this session.')
    ...
    lines.push('Restart Altaris to use it.')
  }
...
function ProviderChooser({
  ...
  const helperText = canUseCodexOAuth
    ? 'Save a provider profile without editing environment variables first. Codex profiles backed by env, auth.json, or Altaris secure storage can switch this session immediately when validation succeeds.'
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .altaris-profile.json and return to normal startup',
    })
  }
...
function CodexOAuthStep({
  ...
  const handleAuthenticated = React.useCallback(async (
    ...
    if (!env) {
      throw new Error(
        'Codex OAuth succeeded, but Altaris could not build a Codex profile from the stored credentials.',
      )
    }
  ...
  return (
    <Dialog title="Codex OAuth" onCancel={onBack}>
      <Box flexDirection="column" gap={1}>
        <Text>
          Finish signing in with ChatGPT in your browser. Altaris will store
        ...
        {status.browserOpened === false ? (
          ...
          <Text dimColor>
            Browser opened. Complete the sign-in there, then Altaris will
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart Altaris to go back to normal startup.`, {
                display: 'system',
              })
            } else {
