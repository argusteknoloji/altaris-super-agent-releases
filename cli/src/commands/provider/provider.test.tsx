...
const ORIGINAL_SIMPLE_ENV = process.env.ALTARIS_SIMPLE
...
afterEach(() => {
  ...
  if (ORIGINAL_SIMPLE_ENV === undefined) {
    ...
    process.env.ALTARIS_SIMPLE = ORIGINAL_SIMPLE_ENV
  }
...
test('buildProfileSaveMessage maps provider fields without echoing secrets', () => {
  const message = buildProfileSaveMessage(
    ...
    'D:/codings/Opensource/altaris/.altaris-profile.json',
  )
...
test('buildProfileSaveMessage reflects immediate Codex activation for existing credentials', () => {
  ...
  expect(message).toContain('Altaris switched to it for this session.')
  expect(message).not.toContain('Restart Altaris to use it.')
})
...
test('explicitly declared env takes precedence over applySavedProfileToCurrentSession', async () => {
  ...
  const processEnv: NodeJS.ProcessEnv = {
    ...
    ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID: 'provider_old',
  }
  ...
  expect(processEnv.ALTARIS_USE_OPENAI).toBe('1')
  ...
  expect(processEnv.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED).toBeUndefined()
  expect(processEnv.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID).toBeUndefined()
})
