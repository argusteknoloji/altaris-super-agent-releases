...
export function getInstallationPath(): string {
  ...
  if (isWindows) {
    ...
    const windowsPath = join(homeDir, '.local', 'bin', 'altaris.exe');
    // Replace forward slashes with backslashes for Windows display
    return windowsPath.replace(/\//g, '\\');
  }
  return '~/.local/bin/altaris';
}
...
function Install({
    ...
    async function run() {
      try {
        ...
        if (result.lockFailed) {
          throw new Error('Could not install - another process is currently installing Altaris. Please try again in a moment.');
        }
  ...
  useEffect(() => {
    if (state.type === 'success') {
      ...
      setTimeout(onDone, 2000, 'Altaris installation completed successfully', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
      setTimeout(onDone, 3000, 'Altaris installation failed', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      {state.type === 'checking' && <Text color="altaris">Checking installation status...</Text>}

      {state.type === 'cleaning-npm' && <Text color="warning">Cleaning up old npm installations...</Text>}

      {state.type === 'installing' && <Text color="altaris">
          Installing Altaris native build {state.version}...
        </Text>}

      {state.type === 'setting-up' && <Text color="altaris">Setting up launcher and shell integration...</Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              Altaris successfully installed!
            </Text>
          </Box>
          <Box marginLeft={2} flexDirection="column" gap={1}>
            {state.version !== 'current' && <Box>
                <Text dimColor>Version: </Text>
                <Text color="altaris">{state.version}</Text>
              </Box>}
            <Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install Altaris native build',
