...
const originalClaudeCodeNewInit = process.env.ALTARIS_NEW_INIT
...
afterEach(() => {
  ...
  if (originalClaudeCodeNewInit === undefined) {
    delete process.env.ALTARIS_NEW_INIT
  } else {
    process.env.ALTARIS_NEW_INIT = originalClaudeCodeNewInit
  }
})

test('NEW_INIT prompt preserves existing root ALTARIS.md by default', async () => {
  process.env.ALTARIS_NEW_INIT = '1'
  ...
  expect(String(blocks[0]?.text)).toContain(
    'checked-in root `ALTARIS.md` and does NOT already have a root `AGENTS.md`',
  )
  expect(String(blocks[0]?.text)).toContain(
    'do NOT silently create a second root instruction file',
  )
  expect(String(blocks[0]?.text)).toContain(
    'update the existing root `ALTARIS.md` in place by default',
  )
})
