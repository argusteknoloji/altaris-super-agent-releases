...
Manage the Altaris project wiki stored in .altaris/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized Altaris wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `Altaris wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the Altaris wiki.`,
    '',
    `Title: ${result.title}`,
