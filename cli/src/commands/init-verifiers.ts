...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - Altaris Chrome Extension MCP (browser-use via Altaris's Chrome extension)
...
2. **If NO browser automation tools are detected**, ask if they want to install/configure one:
   ...
   - Options to offer:
     ...
     - **Altaris Chrome Extension** - Uses the Altaris Chrome extension for browser interaction (requires the extension installed in Chrome)
...
4. **If user chooses Chrome DevTools MCP or Altaris Chrome Extension**:
   ...
   - For Altaris Chrome Extension, inform them they need the extension installed from the Chrome Web Store
...
**All verifier skills are created in the project root's \`.altaris/skills/\` directory.** This ensures they are automatically loaded when Altaris runs in the project.

Write the skill file to \`.altaris/skills/<verifier-name>/SKILL.md\`.
...
1. Where each skill was created (always in \`.altaris/skills/\`)
