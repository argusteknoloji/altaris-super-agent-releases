...
describe('project onboarding completion', () => {
  test('is incomplete when neither AGENTS.md nor ALTARIS.md exists', async () => {
    ...
    await runWithCwdOverride(tempDir, async () => {
      ...
      expect(getSteps()[1]?.text).toContain('ALTARIS.md')
    })
  })

  test('is complete when only ALTARIS.md exists', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'project-onboarding-'))
    await writeFile(join(tempDir, 'ALTARIS.md'), '# ALTARIS.md\n')
