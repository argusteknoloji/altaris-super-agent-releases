...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    case 'not_logged_in':
      return 'Please run /login and sign in with your anthropic.com account (not Console).';
    case 'no_remote_environment':
      return 'No cloud environment available. Set one up at https://altaris.ai/code/onboarding?magic=env-setup';
    ...
    case 'github_app_not_installed':
      return 'The Altaris GitHub app must be installed on this repository first.\nhttps://github.com/apps/altaris/installations/new';
    case 'policy_blocked':
 ...
 *   {type:'system', subtype:'hook_progress', stdout:'...'}. Altaris never
 ...
 * altaris.ai URL stays a durable record the user can revisit; TTL handles cleanup.
 */
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      if (!task || task.status !== 'running') {
        ...
        // Session left alive so the altaris.ai URL stays valid — the run_hunt.sh
      ...
      // stableIdle is a prompt-mode completion signal (Altaris stops writing
      ...
      if (result || sessionDone || reviewTimedOut) {
        ...
        // the formatted findings as an assistant event, so the altaris.ai URL
...
/**
 * RemoteAgentTask - Handles remote anthropic.com session execution.
 *
