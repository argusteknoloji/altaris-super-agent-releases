...
// Only ALTARIS_OAUTH_TOKEN was stripped. Fix: explicit allowlist.
...
test('buildChildEnv does not leak ANTHROPIC_API_KEY to child', () => {
  const parentEnv = {
    ...
    ALTARIS_SESSION_ACCESS_TOKEN: 'will-be-overwritten',
  }
...
test('buildChildEnv sets ALTARIS_SESSION_ACCESS_TOKEN from opts', () => {
  const env = buildChildEnv({ PATH: '/usr/bin' }, { ...baseOpts, accessToken: 'my-token' })
  expect(env.ALTARIS_SESSION_ACCESS_TOKEN).toBe('my-token')
})

test('buildChildEnv sets ALTARIS_ENVIRONMENT_KIND to bridge', () => {
  const env = buildChildEnv({ PATH: '/usr/bin' }, baseOpts)
  expect(env.ALTARIS_ENVIRONMENT_KIND).toBe('bridge')
})

test('buildChildEnv does not pass ALTARIS_OAUTH_TOKEN to child', () => {
  const parentEnv = {
    PATH: '/usr/bin',
    ALTARIS_OAUTH_TOKEN: 'oauth-token-to-strip',
  }
  const env = buildChildEnv(parentEnv, baseOpts)
  expect(env.ALTARIS_OAUTH_TOKEN).toBeUndefined()
})
...
test('buildChildEnv sets CCR v2 vars when useCcrV2 is true', () => {
  ...
  expect(env.ALTARIS_USE_CCR_V2).toBe('1')
  expect(env.ALTARIS_WORKER_EPOCH).toBe('42')
})
