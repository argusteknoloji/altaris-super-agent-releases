 ...
 *
 * Used by both `altaris remote-control` (empty session so the user has somewhere to
 ...
 * explicit client action. Both `altaris remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on altaris.ai/code.
 *
