 ...
 *
 * Remote Control requires a altaris.ai subscription (the bridge auths to CCR
 * with the altaris.ai OAuth token). isClaudeAISubscriber() excludes
 ...
 * (setup-token, ALTARIS_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires a altaris.ai subscription. Run `altaris auth login` to sign in with your altaris.ai account.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `altaris setup-token` or ALTARIS_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `altaris auth login` to use Remote Control.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine your organization for Remote Control eligibility. Run `altaris auth login` to refresh your account information.'
    }
 ...
 * altaris.ai frontend routes on `session_*`, while v2 worker endpoints hand out
...
export function checkBridgeMinVersion(): string | null {
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    if (config.minVersion && lt(MACRO.VERSION, config.minVersion)) {
      return `Your version of Altaris (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`altaris update\` to update.`
    }
  }
  return null
}
...
export function isCcrMirrorEnabled(): boolean {
  return feature('CCR_MIRROR')
    ? isEnvTruthy(process.env.ALTARIS_CCR_MIRROR) ||
