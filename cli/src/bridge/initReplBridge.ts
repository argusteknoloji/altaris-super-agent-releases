...
export type InitBridgeOptions = {
   ...
   * stream). Used by CCR mirror mode — local sessions visible on altaris.ai
...
export async function initReplBridge(
  ...
  // 2. Check OAuth — must be signed in with altaris.ai. Runs before the
  ...
  // Cosmetic only (altaris.ai session list); the model never sees it.
  ...
  // auto-started sessions distinguishable in the altaris.ai list before the
  // first prompt.
  ...
  // The env-based path below can ALSO use CCR v2 via ALTARIS_USE_CCR_V2.
  ...
  if (isEnvLessBridgeEnabled() && !perpetual) {
    ...
    if (versionError) {
      ...
      onStateChange?.('failed', 'run `altaris update` to upgrade')
      return null
    }
    logForDebugging(
  ...
  let workerType: BridgeWorkerType = 'altaris'
  if (feature('KAIROS')) {
...
function deriveTitle(raw: string): string | undefined {
  ...
  // Collapse newlines/tabs — titles are single-line in the altaris.ai list.
