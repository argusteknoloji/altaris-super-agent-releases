 ...
 * (oauth-authed, same store), write to ~/.altaris/uploads/{sessionId}/, and
 * return @path refs to prepend. Altaris's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches Altaris, just without @path.
 */
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // ALTARIS_CUSTOM_OAUTH_URL — keep it inside the try so a bad
