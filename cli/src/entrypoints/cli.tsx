...
// Altaris: polyfill globalThis.File for Node < 20.
...
// Altaris: disable experimental API betas by default.
// Tool search (defer_loading), global cache scope, and context management
// require internal API support not available to external accounts → 500.
// Users can opt-in with ALTARIS_DISABLE_EXPERIMENTAL_BETAS=false.
// eslint-disable-next-line custom-rules/no-top-level-side-effects
process.env.ALTARIS_DISABLE_EXPERIMENTAL_BETAS ??= 'true'
...
if (process.env.ALTARIS_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.ALTARIS_ABLATION_BASELINE) {
  for (const k of ['ALTARIS_SIMPLE', 'ALTARIS_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'ALTARIS_DISABLE_AUTO_MEMORY', 'ALTARIS_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Altaris)`);
    return;
  }
  ...
  // Hydrate GitHub credentials after profile is applied so ALTARIS_USE_GITHUB from profile is available
  {
    const {
  ...
  if (process.argv[2] === '--claude-in-chrome-mcp') {
    ...
    } = await import('../utils/altarisInChrome/mcpServer.js');
    ...
    const {
      runChromeNativeHost
    } = await import('../utils/altarisInChrome/chromeNativeHost.js');
  ...
  // Fast-path for `altaris remote-control` (also accepts legacy `altaris remote` / `altaris sync` / `altaris bridge`):
  ...
  // Fast-path for `altaris daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `altaris ps|logs|attach|kill` and `--bg`/`--background`.
  // Session management against the ~/.altaris/sessions/ registry. Flag
  ...
  // Fast-path for `altaris environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `altaris self-hosted-runner`: headless self-hosted-runner
  ...
  if (process.env.ALTARIS_DISABLE_EARLY_INPUT !== '1') {
    const {
