...
export const ThinkingAdaptiveSchema = lazySchema(() =>
  z
    ...
    .describe('Altaris decides when and how much to think (Opus 4.6+).'),
)
...
export const ThinkingConfigSchema = lazySchema(() =>
  z
    ...
    .describe(
      "Controls Altaris's thinking/reasoning behavior. When set, takes precedence over the deprecated maxThinkingTokens.",
    ),
)
  ...
  z
    .object({
      ...
      capabilities: z
        ...
        .optional()
        .describe(
          "@internal Server capabilities (available when connected). experimental['altaris/channel'] is only present if the server's plugin is on the approved channels allowlist — use its presence to decide whether to show an Enable-channel prompt.",
        ),
    })
  ...
  z
    .object({
      ...
      supportsAdaptiveThinking: z
        ...
        .optional()
        .describe(
          'Whether this model supports adaptive thinking (Altaris decides when and how much to think)',
        ),
      supportsFastMode: z
        .boolean()
        .optional()
  ...
  z
    .object({
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.altaris/agent-memory/<agentType>/, 'project' - .altaris/agent-memory/<agentType>/, 'local' - .altaris/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.altaris/settings.json). " +
        "'project' - Project settings (.altaris/settings.json). " +
        "'local' - Local settings (.altaris/settings.local.json).",
    ),
)
...
export const SDKRateLimitInfoSchema = lazySchema(() =>
  z
    ...
    .describe('Rate limit information for altaris.ai subscription users.'),
)
