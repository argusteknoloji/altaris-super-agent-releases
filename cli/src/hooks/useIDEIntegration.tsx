  ...
  if ($[0] !== autoConnectIdeFlag || $[1] !== ideToInstallExtension || $[2] !== setDynamicMcpConfig || $[3] !== setIDEInstallationState || $[4] !== setShowIdeOnboarding) {
    t1 = () => {
      const addIde = function addIde(ide) {
        ...
        const autoConnectEnabled = (globalConfig.autoConnectIde || autoConnectIdeFlag || isSupportedTerminal() || process.env.ALTARIS_SSE_PORT || ideToInstallExtension || isEnvTruthy(process.env.ALTARIS_AUTO_CONNECT_IDE)) && !isEnvDefinedFalsy(process.env.ALTARIS_AUTO_CONNECT_IDE);
