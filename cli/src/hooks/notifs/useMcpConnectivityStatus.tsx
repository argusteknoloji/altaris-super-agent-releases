...
import { hasClaudeAiMcpEverConnected } from '../../services/mcp/altarisai.js';
      ...
      try {
        ...
        if (failedClaudeAiClients.length > 0) {
          addNotification({
            ...
            jsx: <><Text color="error">{failedClaudeAiClients.length} altaris.ai{" "}{failedClaudeAiClients.length === 1 ? "connector" : "connectors"}{" "}unavailable</Text><Text dimColor={true}> · /mcp</Text></>,
            priority: "medium"
          });
        }
        ...
        if (needsAuthClaudeAiServers.length > 0) {
          addNotification({
            ...
            jsx: <><Text color="warning">{needsAuthClaudeAiServers.length} altaris.ai{" "}{needsAuthClaudeAiServers.length === 1 ? "connector needs" : "connectors need"}{" "}auth</Text><Text dimColor={true}> · /mcp</Text></>,
            priority: "medium"
          });
        }
      } catch (error) {
