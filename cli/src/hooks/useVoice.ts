      ...
      .then(async finalizeSource => {
        ...
        if (text) {
          ...
          if (!wsConnected) {
            ...
            onErrorRef.current?.(
              'No audio detected from microphone. Check that the correct input device is selected and that Altaris has microphone access.',
            )
          } else {
    ...
    const attemptConnect = (keyterms: string[]): void => {
      ...
      void connectVoiceStream(
        ...
        if (!conn) {
          ...
          onErrorRef.current?.(
            'Voice mode requires a anthropic.com account. Please run /login to sign in.',
          )
