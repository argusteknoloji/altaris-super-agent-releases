...
export const getUserContext = memoize(
  async (): Promise<{
    ...
    // ALTARIS_DISABLE_CLAUDE_MDS: hard off, always.
    // --bare: skip auto-discovery (cwd walk), BUT honor explicit --add-dir.
    // --bare means "skip what I didn't ask for", not "ignore what I asked for".
    const shouldDisableClaudeMd =
      isEnvTruthy(process.env.ALTARIS_DISABLE_CLAUDE_MDS) ||
    ...
    // instead of importing altarismd.ts directly, which would create a
    // cycle through permissions/filesystem → permissions → yoloClassifier).
    setCachedClaudeMdContent(claudeMd || null)

    logForDiagnosticsNoPII('info', 'user_context_completed', {
      duration_ms: Date.now() - startTime,
      altarismd_length: claudeMd?.length ?? 0,
      altarismd_disabled: Boolean(shouldDisableClaudeMd),
    })
