...
/**
 * Altaris parses history for pasted content references to match back to
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by Altaris's Tungsten tool.
  // This prevents verification/test sessions from polluting the user's real command history.
  if (isEnvTruthy(process.env.ALTARIS_SKIP_PROMPT_HISTORY)) {
    return
  }
