...
/**
 * Returns a altaris config directory path for a given source.
 */
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.altaris', dir)
    case 'userSettings':
...
/**
 * Parse paths frontmatter from a skill, using the same format as ALTARIS.md rules.
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    const managedSkillsDir = join(getManagedFilePath(), '.altaris', 'skills')
    ...
    const [
      ...
      isEnvTruthy(process.env.ALTARIS_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.altaris', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.altaris/skills from
 ...
 * of ALTARIS.md conditional rules.
 *
