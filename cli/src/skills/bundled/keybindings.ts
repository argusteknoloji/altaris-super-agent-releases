...
const FILE_FORMAT_EXAMPLE: KeybindingsSchemaType = {
  $schema: 'https://www.schemastore.org/altaris-keybindings.json',
...
const SECTION_INTRO = [
  '# Keybindings Skill',
  '',
  'Create or modify `~/.altaris/keybindings.json` to customize keyboard shortcuts.',
  '',
  '## CRITICAL: Read Before Write',
  '',
  '**Always read `~/.altaris/keybindings.json` first** (it may not exist yet). Merge changes with existing bindings — never replace the entire file.',
  '',
...
const SECTION_DOCTOR = [
  ...
  '',
  'The `/doctor` command includes a "Keybinding Configuration Issues" section that validates `~/.altaris/keybindings.json`.',
  '',
  ...
  'Location: ~/.altaris/keybindings.json',
...
export function registerKeybindingsSkill(): void {
  registerBundledSkill({
    ...
    description:
      'Use when the user wants to customize keyboard shortcuts, rebind keys, add chord bindings, or modify ~/.altaris/keybindings.json. Examples: "rebind ctrl+s", "add a chord shortcut", "change the submit key", "customize keybindings".',
    allowedTools: ['Read'],
    userInvocable: false,
