...
function writeSkill(rootDir: string, skillPath: string): void {
  const skillDir = join(rootDir, '.altaris', 'skills', ...skillPath.split('/'))
...
test('loads flat and nested skills with colon namespaces', async () => {
  const configDir = mkdtempSync(join(tmpdir(), 'altaris-skills-'))
  ...
  try {
    ...
    assert.equal(nestedSkill.skillRoot, join(configDir, '.altaris', 'skills', 'git', 'commit'))
    ...
    assert.equal(
      deepSkill.skillRoot,
      join(configDir, '.altaris', 'skills', 'frontend', 'react', 'form'),
    )
  } finally {
