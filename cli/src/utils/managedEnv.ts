 ...
 * env vars that the remote's ~/.altaris settings.env MUST NOT clobber (see
 ...
 * ALTARIS_PROVIDER_MANAGED_BY_HOST in spawn env), strip
 * provider-selection / model-default vars from settings-sourced env so a
 * user's ~/.altaris/settings.json can't redirect requests away from the
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.ALTARIS_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 *
 * - userSettings (~/.altaris/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.altaris.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.altaris/settings.json env (gh#217). policy/flag
  ...
  // flagSettings env applied. Eligibility reads ALTARIS_USE_BEDROCK,
  ...
  // when ALTARIS_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * ALTARIS_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
