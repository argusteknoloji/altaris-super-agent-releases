...
export const getGlobalClaudeFile = memoize((): string => {
  ...
  // Default to .altaris.json. Fall back to .altaris.json only if the new
  ...
  const newFilename = `.altaris${oauthSuffix}.json`
  const legacyFilename = `.altaris${oauthSuffix}.json`
  if (
 ...
 * If ALTARIS_HOST_PLATFORM is set to a valid platform value, that overrides
...
export function getHostPlatformForAnalytics(): Platform {
  const override = process.env.ALTARIS_HOST_PLATFORM
