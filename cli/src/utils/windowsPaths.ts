...
export const findGitBashPath = memoize((): string => {
  if (process.env.ALTARIS_GIT_BASH_PATH) {
    if (checkPathExists(process.env.ALTARIS_GIT_BASH_PATH)) {
      return process.env.ALTARIS_GIT_BASH_PATH
    }
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.error(
      `Altaris was unable to find ALTARIS_GIT_BASH_PATH path "${process.env.ALTARIS_GIT_BASH_PATH}"`,
    )
  ...
  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.error(
    'Altaris on Windows requires git-bash (https://git-scm.com/downloads/win). If installed but not in PATH, set environment variable pointing to your bash.exe, similar to: ALTARIS_GIT_BASH_PATH=C:\\Program Files\\Git\\bin\\bash.exe',
  )
  // eslint-disable-next-line custom-rules/no-process-exit
  process.exit(1)
})
