...
describe('hydrateGithubModelsTokenFromSecureStorage', () => {
  const orig = {
    ...
    ALTARIS_GITHUB_TOKEN_HYDRATED:
      process.env.ALTARIS_GITHUB_TOKEN_HYDRATED,
    ALTARIS_SIMPLE: process.env.ALTARIS_SIMPLE,
  }
  ...
  test('sets GITHUB_TOKEN from secure storage when USE_GITHUB and env token empty', async () => {
    ...
    expect(process.env.ALTARIS_GITHUB_TOKEN_HYDRATED).toBe('1')
  })

  test('does not override existing GITHUB_TOKEN', async () => {
    process.env.ALTARIS_USE_GITHUB = '1'
    process.env.GITHUB_TOKEN = 'already'
    delete process.env.ALTARIS_GITHUB_TOKEN_HYDRATED
    ...
    expect(process.env.ALTARIS_GITHUB_TOKEN_HYDRATED).toBeUndefined()
  })
})
