 ...
 * rule (src/ALTARIS.md): the dispatcher closure must persist across calls so
...
function formatLockHeld(holder: string): string {
  return `Computer use is in use by another Altaris session (${holder.slice(0, 8)}…). Wait for that session to finish or run /exit there.`;
}
    ...
    acquireCuLock: async () => {
      ...
      if (r.fresh) {
        ...
        tuc().sendOSNotification?.({
          message: escRegistered ? 'Altaris is using your computer · press Esc to stop' : 'Altaris is using your computer · press Ctrl+C to stop',
