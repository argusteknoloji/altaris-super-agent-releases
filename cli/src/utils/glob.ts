...
export async function glob(
  ...
  // --no-ignore: don't respect .gitignore (default true, set ALTARIS_GLOB_NO_IGNORE=false to respect .gitignore)
  // --hidden: include hidden files (default true, set ALTARIS_GLOB_HIDDEN=false to exclude)
  // Note: use || instead of ?? to treat empty string as unset (defaulting to true)
  const noIgnore = isEnvTruthy(process.env.ALTARIS_GLOB_NO_IGNORE || 'true')
  const hidden = isEnvTruthy(process.env.ALTARIS_GLOB_HIDDEN || 'true')
  const args = [
    '--files',
    '--glob',
    searchPattern,
