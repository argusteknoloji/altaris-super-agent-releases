...
import { getLargeMemoryFiles, MAX_MEMORY_CHARACTER_COUNT, type MemoryFileInfo } from './altarismd.js';
...
const claudeAiSubscriberExternalTokenNotice: StatusNoticeDefinition = {
  ...
  render: () => {
    ...
    return <Box flexDirection="row" marginTop={1}>
        <Text color="warning">{figures.warning}</Text>
        <Text color="warning">
          Auth conflict: Using {authTokenInfo.source} instead of Altaris account
  ...
  render: () => {
    ...
    return <Box flexDirection="row" marginTop={1}>
        ...
        <Text color="warning">
          ...
          Either unset {apiKeySource}, or run `altaris /logout`.
        </Text>
      </Box>;
  }
};
  ...
  render: () => {
    ...
    return <Box flexDirection="column" marginTop={1}>
        ...
        <Box flexDirection="column" marginLeft={3}>
          <Text color="warning">
            · Trying to use{' '}
            {authTokenInfo.source === 'altaris.ai' ? 'altaris.ai' : authTokenInfo.source}
            ?{' '}
            {apiKeySource === 'ANTHROPIC_API_KEY' ? 'Unset the ANTHROPIC_API_KEY environment variable, or altaris /logout then say "No" to the API key approval before login.' : apiKeySource === 'apiKeyHelper' ? 'Unset the apiKeyHelper setting.' : 'altaris /logout'}
          </Text>
          <Text color="warning">
            · Trying to use {apiKeySource}?{' '}
            {authTokenInfo.source === 'altaris.ai' ? 'altaris /logout to sign out of altaris.ai.' : `Unset the ${authTokenInfo.source} environment variable.`}
          </Text>
        </Box>
      </Box>;
  }
  ...
  render: () => {
    ...
    return <Box flexDirection="row" gap={1} marginLeft={1}>
        ...
        <Text>
          ...
          <Text bold>https://docs.altaris.com/s/altaris-jetbrains</Text>
        </Text>
      </Box>;
  }
};
