 ...
 * one into the other. Surfaced as warnings by `altaris plugin validate` since
...
export async function validatePluginManifest(
  ...
  // them. `altaris plugin validate` is a developer tool — authors running it
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Altaris ` +
  ...
  if (result.success) {
    ...
    // but the anthropic.com marketplace sync rejects non-kebab names. Surfacing
    // this here lets authors catch it in CI before the sync fails on them.
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Altaris accepts ` +
          `it, but the anthropic.com marketplace sync requires kebab-case ` +
 ...
 * for the load path, but authors running `altaris plugin validate` want a hard
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and Altaris ` +
