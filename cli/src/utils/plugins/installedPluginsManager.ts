 ...
 *
 * The enabled/disabled state remains in .altaris/settings.json for per-repo control.
 *
 ...
 *
 * Legacy cache structure: ~/.altaris/plugins/cache/{plugin-name}/
 * Versioned cache structure: ~/.altaris/plugins/cache/{marketplace}/{plugin}/{version}/
 *
...
function migrateV1ToV2(v1Data: InstalledPluginsFileV1): InstalledPluginsFileV2 {
  ...
  for (const [pluginId, plugin] of Object.entries(v1Data.plugins)) {
    // V2 format uses versioned cache path: ~/.altaris/plugins/cache/{marketplace}/{plugin}/{version}
 ...
 *
 * getOriginalCwd() (not getCwd()) because "current project" is where Altaris
