 ...
 * When ALTARIS_PLUGIN_USE_ZIP_CACHE is enabled and ALTARIS_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.ALTARIS_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires ALTARIS_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.ALTARIS_PLUGIN_CACHE_DIR
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, anthropics/altaris#13893
  try {
