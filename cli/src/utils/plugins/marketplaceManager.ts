/**
 * Marketplace manager for Altaris plugins
 *
 ...
 *
 * Reads the configuration file at ~/.altaris/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.altaris/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.altaris/plugins/marketplaces/company-plugins",
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Altaris bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.altaris/plugins/known_marketplaces.json,
 ...
 *
 * Pulls latest changes with a configurable timeout (default 120s, override via ALTARIS_PLUGIN_GIT_TIMEOUT_MS).
...
function getPluginGitTimeoutMs(): number {
  const envValue = process.env.ALTARIS_PLUGIN_GIT_TIMEOUT_MS
  if (envValue) {
...
function enhanceGitPullErrorMessages(result: {
  ...
  if (result.error?.includes('timed out')) {
    ...
    return {
      ...result,
      stderr: `Git pull timed out after ${timeoutSec}s. Try increasing the timeout via ALTARIS_PLUGIN_GIT_TIMEOUT_MS environment variable.\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 *
 * Clones a git repository with a configurable timeout (default 120s, override via ALTARIS_PLUGIN_GIT_TIMEOUT_MS)
...
export async function gitClone(
  ...
  if (result.error?.includes('timed out')) {
    return {
      ...result,
      stderr: `Git clone timed out after ${Math.round(timeoutMs / 1000)}s. The repository may be too large for the current timeout. Set ALTARIS_PLUGIN_GIT_TIMEOUT_MS to increase it (e.g., 300000 for 5 minutes).\n\nOriginal error: ${result.stderr}`,
    }
  }
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'Altaris-Code-Plugin-Manager',
  }
 ...
 * The configuration is saved to ~/.altaris/plugins/known_marketplaces.json.
 *
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: altaris plugin disable <plugin>@${name}`,
    )
  }
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Altaris version. Run 'altaris marketplace remove ${name}' and ` +
...
export async function refreshAllMarketplaces(): Promise<void> {
  ...
  for (const [name, entry] of Object.entries(config)) {
    ...
    // hits this path on `altaris plugin marketplace update` (no name arg).
  ...
  try {
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: altaris plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      if (source.source === 'github') {
        ...
        if (isEnvTruthy(process.env.ALTARIS_REMOTE)) {
      ...
      try {
        ...
        const reason =
          name === 'altaris-plugins'
            ? `We've deprecated "altaris-plugins" in favor of "claude-plugins-official".`
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: altaris plugin marketplace remove "${name}"`,
        )
      }
