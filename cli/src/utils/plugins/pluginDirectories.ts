 ...
 * - CLI flag: --cowork
 * - Environment variable: ALTARIS_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via ALTARIS_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable ALTARIS_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.ALTARIS_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. ALTARIS_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.altaris/plugins or ~/.altaris/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when ALTARIS_PLUGIN_CACHE_DIR is set via settings.json
  // `env` (not shell), ~ is not expanded by the shell. Without this, a value
  // like "~/.altaris/plugins" becomes a literal `~` directory created in the
  // cwd of every project (gh-30794 / CC-212).
  const envOverride = process.env.ALTARIS_PLUGIN_CACHE_DIR
  if (envOverride) {
 ...
 * image and point ALTARIS_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.ALTARIS_PLUGIN_SEED_DIR
  if (!raw) return []
