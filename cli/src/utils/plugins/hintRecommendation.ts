 ...
 * `<altaris-hint />` tag to stderr (detected by the Bash/PowerShell tools).
 *
 * State persists in GlobalConfig.altarisHints — a show-once record per
...
/**
 * Hard cap on `altarisHints.plugin[]` — bounds config growth. Each shown
...
export function maybeRecordPluginHint(hint: ClaudeCodeHint): void {
  ...
  const state = getGlobalConfig().altarisHints
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.altarisHints?.plugin ?? []
    if (existing.includes(pluginId)) return current
    return {
      ...current,
      altarisHints: {
        ...current.altarisHints,
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.altarisHints?.disabled) return current
    return {
      ...current,
      altarisHints: { ...current.altarisHints, disabled: true },
    }
  })
}
