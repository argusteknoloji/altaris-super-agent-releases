...
// imports above. Named (not wildcard) per ALTARIS.md style; no collisions
...
function getEntrypoint(): string | undefined {
  return process.env.ALTARIS_ENTRYPOINT
}
...
class Project {
   ...
   * ALTARIS_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.ALTARIS_SKIP_PROMPT_HISTORY)
    )
  }
 ...
 *
 * Streaming (altaris.ts:~2024) emits one AssistantMessage per content_block_stop
...
/**
 * Append a periodic task summary for `altaris ps`. Unlike ai-title this is
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.ALTARIS_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // ALTARIS_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.ALTARIS_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.ALTARIS_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
