...
const PROFILE_ENV_APPLIED_FLAG = 'ALTARIS_PROVIDER_PROFILE_ENV_APPLIED'
const PROFILE_ENV_APPLIED_ID = 'ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID'
...
function hasProviderSelectionFlags(
  ...
  return (
    processEnv.ALTARIS_USE_OPENAI !== undefined ||
    processEnv.ALTARIS_USE_GEMINI !== undefined ||
    processEnv.ALTARIS_USE_MISTRAL !== undefined ||
    processEnv.ALTARIS_USE_GITHUB !== undefined ||
    processEnv.ALTARIS_USE_BEDROCK !== undefined ||
    processEnv.ALTARIS_USE_VERTEX !== undefined ||
    processEnv.ALTARIS_USE_FOUNDRY !== undefined
  )
}
 ...
 * to run (a model id). A bare `ALTARIS_USE_OPENAI=1` with nothing else
...
function hasCompleteProviderSelection(
  ...
  if (processEnv.ALTARIS_USE_OPENAI !== undefined) {
    return (
  ...
  if (processEnv.ALTARIS_USE_GEMINI !== undefined) {
    return (
  ...
  if (processEnv.ALTARIS_USE_MISTRAL !== undefined) {
    return (
  ...
  if (processEnv.ALTARIS_USE_GITHUB !== undefined) {
    return (
...
export function clearProviderProfileEnvFromProcessEnv(
  ...
  delete processEnv.ALTARIS_USE_OPENAI
  delete processEnv.ALTARIS_USE_GEMINI
  delete processEnv.ALTARIS_USE_MISTRAL
  delete processEnv.ALTARIS_USE_GITHUB
  delete processEnv.ALTARIS_USE_BEDROCK
  delete processEnv.ALTARIS_USE_VERTEX
  delete processEnv.ALTARIS_USE_FOUNDRY
...
export function applyProviderProfileToProcessEnv(profile: ProviderProfile): void {
  ...
  if (profile.provider === 'mistral') {
    process.env.ALTARIS_USE_MISTRAL = '1'
...
export function applyActiveProviderProfileFromConfig(
  ...
  if (!options?.force && (hasCompleteProviderSelection(processEnv) || processEnv[PROFILE_ENV_APPLIED_FLAG] === '1')) {
    ...
    // rather than hasProviderSelectionFlags alone. A bare ALTARIS_USE_*=1
