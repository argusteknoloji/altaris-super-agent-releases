...
export function isFastModeEnabled(): boolean {
  ...
  return !isEnvTruthy(process.env.ALTARIS_DISABLE_FAST_MODE)
}
...
export function getFastModeUnavailableReason(): string | null {
  ...
  if (
    ...
    return 'Fast mode requires the native binary · Install from: https://altaris.com/product/altaris'
  }
  ...
  if (orgStatus.status === 'disabled') {
    if (
      ...
      // endpoint. We add ALTARIS_SKIP_FAST_MODE_NETWORK_ERRORS=1 to
      // bypass this check in the CC binary. This is OK since we have
      // another check in the API to error out when disabled by org.
      if (isEnvTruthy(process.env.ALTARIS_SKIP_FAST_MODE_NETWORK_ERRORS)) {
        return null
      }
    }
...
async function fetchFastModeStatus(
  ...
  const endpoint = `${getOauthConfig().BASE_API_URL}/api/altaris_penguin_mode`
