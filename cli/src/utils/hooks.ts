...
export function getSessionEndHookTimeoutMs(): number {
  const raw = process.env.ALTARIS_SESSIONEND_HOOKS_TIMEOUT_MS
 ...
 * .altaris/settings.json. This is a defense-in-depth security measure.
 *
 ...
 * (POSIX path conversion, .sh auto-prepend, ALTARIS_SHELL_PREFIX).
...
async function execCommandHook(
  ...
  // ALTARIS_SHELL_PREFIX wraps the command via POSIX quoting
  ...
  // a ALTARIS_PS_SHELL_PREFIX (or shell-aware prefix) is a follow-up.
  const finalCommand =
    !isPowerShell && process.env.ALTARIS_SHELL_PREFIX
      ? formatShellPrefixCommand(process.env.ALTARIS_SHELL_PREFIX, command)
      : command
...
/**
 * Execute InstructionsLoaded hooks when an instruction file (ALTARIS.md or
 * .altaris/rules/*.md) is loaded into context. Fire-and-forget — this hook is
 * for observability/audit only and does not support blocking.
 *
 * Dispatch sites:
 * - Eager load at session start (getMemoryFiles in altarismd.ts)
 * - Eager reload after compaction (getMemoryFiles cache cleared by
 *   runPostCompactCleanup; next call reports load_reason: 'compact')
 * - Lazy load when Altaris touches a file that triggers nested ALTARIS.md or
