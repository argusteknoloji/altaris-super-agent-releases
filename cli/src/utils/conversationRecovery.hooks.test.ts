...
const originalSimple = process.env.ALTARIS_SIMPLE
...
async function writeJsonl(entry: unknown): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'altaris-conversation-recovery-hooks-'))
  tempDirs.push(dir)
...
afterEach(async () => {
  mock.restore()
  process.env.ALTARIS_SIMPLE = originalSimple
