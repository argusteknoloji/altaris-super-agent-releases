...
function getTokenFromFileDescriptor(): string | null {
  ...
  const fdEnv = process.env.ALTARIS_WEBSOCKET_AUTH_FILE_DESCRIPTOR
  if (!fdEnv) {
  ...
  if (Number.isNaN(fd)) {
    logForDebugging(
      `ALTARIS_WEBSOCKET_AUTH_FILE_DESCRIPTOR must be a valid file descriptor number, got: ${fdEnv}`,
      { level: 'error' },
    )
 ...
 *
 * Priority order:
 *  1. Environment variable (ALTARIS_SESSION_ACCESS_TOKEN) — set at spawn time,
 ...
 *  2. File descriptor (legacy path) — ALTARIS_WEBSOCKET_AUTH_FILE_DESCRIPTOR,
 *     read once and cached.
 *  3. Well-known file — CLAUDE_SESSION_INGRESS_TOKEN_FILE env var path, or
 *     /home/claude/.altaris/remote/.session_ingress_token. Covers subprocesses
 *     that can't inherit the FD.
 */
export function getSessionIngressAuthToken(): string | null {
  // 1. Check environment variable
  const envToken = process.env.ALTARIS_SESSION_ACCESS_TOKEN
  if (envToken) {
    return envToken
  }
...
export function getSessionIngressAuthHeaders(): Record<string, string> {
  ...
  if (token.startsWith('sk-ant-sid')) {
    ...
    const orgUuid = process.env.ALTARIS_ORGANIZATION_UUID
    if (orgUuid) {
...
export function updateSessionIngressAuthToken(token: string): void {
  process.env.ALTARIS_SESSION_ACCESS_TOKEN = token
}
