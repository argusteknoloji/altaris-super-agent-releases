      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[Altaris Browser Bridge] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[Altaris Browser Bridge] No compatible browser found')
    return false
  }
...
function getSocketName(): string {
  // NOTE: This must match the one used in the Altaris Browser Bridge MCP
