 ...
 *
 * Priority order:
 *   ALTARIS_NO_FLICKER=0  → always off
 *   ALTARIS_NO_FLICKER=1  → always on (overrides tmux -CC guard too)
...
export function isFullscreenEnvEnabled(): boolean {
  ...
  if (isEnvDefinedFalsy(process.env.ALTARIS_NO_FLICKER)) return false
  // Explicit env opt-in overrides everything including tmux -CC.
  if (isEnvTruthy(process.env.ALTARIS_NO_FLICKER)) return true
  ...
  if (isTmuxControlMode()) {
    if (!loggedTmuxCcDisable) {
      ...
      logForDebugging(
        'fullscreen disabled: tmux -CC (iTerm2 integration mode) detected · set ALTARIS_NO_FLICKER=1 to override',
      )
    }
    return false
  }
 ...
 * Set ALTARIS_DISABLE_MOUSE=1 to keep alt-screen + virtualized scroll
 ...
 *
 * Compare with ALTARIS_NO_FLICKER=0 which is all-or-nothing — it also
 * disables alt-screen and virtualized scrollback.
 */
export function isMouseTrackingEnabled(): boolean {
  return !isEnvTruthy(process.env.ALTARIS_DISABLE_MOUSE)
}

/**
 * Whether mouse click handling is disabled (clicks/drags ignored, wheel still
 * works). Set ALTARIS_DISABLE_MOUSE_CLICKS=1 to prevent accidental clicks
 * from triggering cursor positioning, text selection, or message expansion.
 *
 * Fullscreen-specific — only reachable when ALTARIS_NO_FLICKER is active.
 */
export function isMouseClicksDisabled(): boolean {
  return isEnvTruthy(process.env.ALTARIS_DISABLE_MOUSE_CLICKS)
}
