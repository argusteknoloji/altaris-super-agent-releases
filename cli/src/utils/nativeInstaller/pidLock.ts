 ...
 *
 * This module provides PID-based locking for running Altaris versions.
...
/**
 * Validate that a running process is actually a Altaris process
...
function isClaudeProcess(pid: number, expectedExecPath: string): boolean {
  ...
  // This handles test environments where the command might not contain 'altaris'
  if (pid === process.pid) {
    return true
  }
  ...
  try {
    ...
    // Check if the command contains 'altaris' or the expected exec path
    const normalizedCommand = command.toLowerCase()
    const normalizedExecPath = expectedExecPath.toLowerCase()

    return (
      normalizedCommand.includes('altaris') ||
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a Altaris process?
  // This helps with PID reuse scenarios
  if (!isClaudeProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be Altaris - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a Altaris process (to handle PID reuse scenarios)
