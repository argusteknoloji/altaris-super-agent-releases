...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'altaris.exe' : 'altaris'
}
...
function getBaseDirectories() {
  ...
  return {
    ...
    versions: join(getXDGDataHome(), 'altaris', 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), 'altaris', 'staging'),

    // State directories
    locks: join(getXDGStateHome(), 'altaris', 'locks'),
...
async function performVersionUpdate(
  ...
  // Create direct symlink from ~/.local/bin/altaris to the version binary
...
export async function checkInstall(
  ...
  // Check if altaris executable exists and is valid.
  ...
  if (isWindows) {
    ...
    if (!(await isPossibleClaudeBinary(dirs.executable))) {
      messages.push({
        message: `installMethod is native, but altaris command is missing or invalid at ${dirs.executable}`,
        userActionRequired: true,
        type: 'error',
      })
    }
    ...
    try {
      ...
      if (!(await isPossibleClaudeBinary(absoluteTarget))) {
        messages.push({
          message: `Altaris symlink points to missing or invalid binary: ${target}`,
          userActionRequired: true,
          type: 'error',
        })
      }
      ...
      if (isENOENT(e)) {
        messages.push({
          message: `installMethod is native, but altaris command not found at ${dirs.executable}`,
          userActionRequired: true,
          type: 'error',
        })
      } else {
        ...
        if (!(await isPossibleClaudeBinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid Altaris binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
  ...
  if (getPlatform().startsWith('win32')) {
    ...
    try {
      ...
      for (const file of files) {
        if (!/^altaris\.exe\.old\.\d+$/.test(file)) continue
        try {
...
/**
 * Remove the altaris symlink from the executable directory
...
export async function removeInstalledSymlink(): Promise<void> {
  ...
  try {
    ...
    logForDebugging(`Removed altaris symlink at ${dirs.executable}`)
  } catch (error) {
    if (isENOENT(error)) {
      return
    }
    logError(new Error(`Failed to remove altaris symlink: ${error}`))
  }
}

/**
 * Clean up old altaris aliases from shell configuration files
  ...
  for (const [shellType, configFile] of Object.entries(configMap)) {
    try {
      ...
      if (hadAlias) {
        ...
        messages.push({
          message: `Removed altaris alias from ${configFile}. Run: unalias altaris`,
          userActionRequired: true,
          type: 'alias',
        })
        logForDebugging(`Cleaned up altaris alias from ${shellType} config`)
      }
    } catch (error) {
      logError(error)
      messages.push({
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, 'altaris.cmd')
      const binPs1 = join(globalPrefix, 'altaris.ps1')
      const binExe = join(globalPrefix, 'altaris')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'altaris')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove @anthropic-ai/altaris
  ...
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== '@anthropic-ai/altaris') {
  ...
  // Preserve compatibility with pre-migration installs under ~/.altaris/local.
  const localInstallDirs = Array.from(
    new Set([join(getClaudeConfigHomeDir(), 'local'), join(homedir(), '.altaris', 'local')]),
  )
