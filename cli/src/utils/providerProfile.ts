...
export const PROFILE_FILE_NAME = '.altaris-profile.json'
...
export function hasExplicitProviderSelection(
  ...
  if (processEnv.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED === '1') {
    return true
  }

  return (
    isEnvTruthy(processEnv.ALTARIS_USE_OPENAI) ||
    isEnvTruthy(processEnv.ALTARIS_USE_GITHUB) ||
    isEnvTruthy(processEnv.ALTARIS_USE_GEMINI) ||
    isEnvTruthy(processEnv.ALTARIS_USE_MISTRAL) ||
    isEnvTruthy(processEnv.ALTARIS_USE_BEDROCK) ||
    isEnvTruthy(processEnv.ALTARIS_USE_VERTEX) ||
    isEnvTruthy(processEnv.ALTARIS_USE_FOUNDRY)
  )
}
...
export async function buildLaunchEnv(options: {
  ...
  if (hasExplicitProviderSelection(processEnv)) {
    for (let provider of PROVIDERS) {
      ...
      const env_key_name = `ALTARIS_USE_${provider.toUpperCase()}`
  ...
  if (options.profile === 'gemini') {
    const env: NodeJS.ProcessEnv = {
      ...processEnv,
      ALTARIS_USE_GEMINI: '1',
    }

    delete env.ALTARIS_USE_OPENAI
    delete env.ALTARIS_USE_GITHUB
  ...
  if (options.profile === 'mistral') {
    const env: NodeJS.ProcessEnv = {
      ...processEnv,
      ALTARIS_USE_MISTRAL: '1',
    }

    delete env.ALTARIS_USE_OPENAI
    delete env.ALTARIS_USE_GITHUB
    delete env.ALTARIS_USE_GEMINI
    delete env.ALTARIS_USE_BEDROCK
    delete env.ALTARIS_USE_VERTEX
    delete env.ALTARIS_USE_FOUNDRY
  ...
  const env: NodeJS.ProcessEnv = {
    ...processEnv,
    ALTARIS_USE_OPENAI: '1',
  }

  delete env.ALTARIS_USE_MISTRAL
  delete env.ALTARIS_USE_BEDROCK
  delete env.ALTARIS_USE_VERTEX
  delete env.ALTARIS_USE_FOUNDRY
  delete env.ALTARIS_USE_GEMINI
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  const profileManagedEnv = processEnv.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED === '1'

  // The legacy single-profile file (~/.altaris-profile.json) is a
  ...
  // and signals completion with ALTARIS_PROVIDER_PROFILE_ENV_APPLIED=1.
  //
  ...
  if (isEnvTruthy(processEnv.ALTARIS_USE_GITHUB)) {
    return processEnv
  }
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.ALTARIS_PROFILE_GOAL),
    processEnv,
...
export async function applySavedProfileToCurrentSession(options: {
  ...
  delete baseEnv.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED
  delete baseEnv.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID
  ...
  const nextEnv = await buildLaunchEnv({
    ...
    goal: normalizeRecommendationGoal(processEnv.ALTARIS_PROFILE_GOAL),
    processEnv: baseEnv,
    getOllamaChatBaseUrl,
  ...
  delete processEnv.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED
  delete processEnv.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID
