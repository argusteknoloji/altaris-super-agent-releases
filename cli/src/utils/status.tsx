...
import { getLargeMemoryFiles, getMemoryFiles, MAX_MEMORY_CHARACTER_COUNT } from './altarismd.js';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(process.env.ALTARIS_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.ALTARIS_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.ALTARIS_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.ALTARIS_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.ALTARIS_CLIENT_KEY
      });
    }
  }
  return properties;
