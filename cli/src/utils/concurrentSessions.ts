...
/**
 * Kind override from env. Set by the spawner (`altaris --bg`, daemon
...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.ALTARIS_SESSION_KIND
...
/**
 * True when this REPL is running inside a `altaris --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `altaris ps` sees everything
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.ALTARIS_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.ALTARIS_SESSION_NAME,
              logPath: process.env.ALTARIS_SESSION_LOG,
              agent: process.env.ALTARIS_AGENT,
            }
          : {}),
      }),
    )
    // --resume / /resume mutates getSessionId() via switchSession. Without
    // this, the PID file's sessionId goes stale and `altaris ps` sparkline
...
/**
 * Push live activity state for `altaris ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.altaris/sessions/ is shared with Windows-native Altaris (symlink
