...
import { queryHaiku } from '../services/api/altaris.js';
...
You should keep it short and simple, ideally no more than 4 words. The branch should always start with "altaris/" and should be all lower case, with words separated by dashes.

Return a JSON object with "title" and "branch" fields.

Example 1: {"title": "Fix login button not working on mobile", "branch": "altaris/fix-mobile-login-button"}
Example 2: {"title": "Update README with installation instructions", "branch": "altaris/update-readme"}
Example 3: {"title": "Improve performance of data processing script", "branch": "altaris/improve-data-processing"}
...
/**
 * Generates a title and branch name for a coding session using Altaris Haiku
...
async function generateTitleAndBranch(description: string, signal: AbortSignal): Promise<TitleAndBranch> {
  ...
  const fallbackBranch = 'altaris/task';
  try {
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('Altaris web sessions require authentication with a anthropic.com account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
      ...
      case 'not_in_repo':
        {
          ...
          throw new TeleportOperationError(`You must run altaris --teleport ${sessionId} from a checkout of ${notInRepoDisplay}.`, chalk.red(`You must run altaris --teleport ${sessionId} from a checkout of ${chalk.bold(notInRepoDisplay)}.\n`));
        }
      case 'mismatch':
        {
          ...
          throw new TeleportOperationError(`You must run altaris --teleport ${sessionId} from a checkout of ${sessionDisplay}.\nThis repo is ${currentDisplay}.`, chalk.red(`You must run altaris --teleport ${sessionId} from a checkout of ${chalk.bold(sessionDisplay)}.\nThis repo is ${chalk.bold(currentDisplay)}.\n`));
        }
      case 'error':
...
/**
 * Creates a remote anthropic.com session with error handling and UI feedback.
...
/**
 * Creates a remote anthropic.com session using the Sessions API.
 *
 * Two source modes:
...
export async function teleportToRemote(options: {
   ...
   * environmentId is set, ALTARIS_OAUTH_TOKEN is auto-injected from the
   ...
   * a new altaris/ branch. Sets allow_unrestricted_git_push on the source and
  ...
  try {
    ...
    if (options.environmentId) {
      ...
      const envVars = {
        ALTARIS_OAUTH_TOKEN: accessToken,
    ...
    if (!gitSource && bundleSeedGateOn) {
      ...
      if (!bundle.success) {
        ...
        const setup = repoInfo ? '. Please setup GitHub on https://altaris.ai/code' : '';
        let msg: string;
