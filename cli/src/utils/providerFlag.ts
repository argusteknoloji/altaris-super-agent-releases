 ...
 *
 * Usage:
 *   altaris --provider openai --model gpt-4o
 *   altaris --provider gemini --model gemini-2.0-flash
 *   altaris --provider mistral --model ministral-3b-latest
 *   altaris --provider ollama --model llama3.2
 *   altaris --provider anthropic   (default, no-op)
 */
 ...
 * Sets the required ALTARIS_USE_* flag and any provider-specific
...
export function applyProviderFlag(
  ...
  switch (provider as ProviderFlagName) {
    ...
    case 'bedrock':
      process.env.ALTARIS_USE_BEDROCK = '1'
      break

    case 'vertex':
      process.env.ALTARIS_USE_VERTEX = '1'
      break
