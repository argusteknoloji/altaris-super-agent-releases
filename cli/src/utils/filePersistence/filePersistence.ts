 ...
 * - Checks environment kind (ALTARIS_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires ALTARIS_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.ALTARIS_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but ALTARIS_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
