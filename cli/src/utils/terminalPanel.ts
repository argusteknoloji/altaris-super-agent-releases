 ...
 * socket (e.g., "claude-panel-a1b2c3d4") holds the shell session. Each Altaris
 ...
 * Altaris while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per Altaris instance (based on session ID)
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to Altaris from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to Altaris ', ';',
