 ...
 * Handles conversions between Altaris's environment and the IDE's environment
 */
...
export interface IDEPathConverter {
  /**
   * Convert path from IDE format to Altaris's local format
   * Used when reading workspace folders from IDE lockfile
   */
  toLocalPath(idePath: string): string

  /**
   * Convert path from Altaris's local format to IDE format
...
/**
 * Converter for Windows IDE + WSL Altaris scenario
 */
