/**
 * Altaris hints protocol.
 *
 * CLIs and SDKs running under Altaris can emit a self-closing
 * `<altaris-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/altaris-hints.md for the vendor-facing spec.
 */
 ...
 */
const HINT_TAG_RE = /^[ \t]*<altaris-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
...
export function extractClaudeCodeHints(
  ...
  if (!output.includes('<altaris-hint')) {
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[altarisHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[altarisHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[altarisHints] dropped hint with empty value')
      return ''
    }
