...
/**
 * Builds a deep link URL for Altaris Desktop to resume a CLI session.
 * Format: altaris://resume?session={sessionId}&cwd={cwd}
 * In dev mode: claude-dev://resume?session={sessionId}&cwd={cwd}
 */
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'claude-dev' : 'altaris'
...
/**
 * Check if Altaris Desktop app is installed.
 * On macOS, checks for /Applications/Altaris.app.
 * On Linux, checks if xdg-open can handle altaris:// protocol.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for Altaris.app in /Applications
    return pathExists('/Applications/Altaris.app')
  } else if (platform === 'linux') {
    // Check if xdg-mime can find a handler for altaris://
...
/**
 * Detect the installed Altaris Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/Altaris.app/Contents/Info.plist',
...
/**
 * Build and open a deep link to resume the current session in Altaris Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'Altaris Desktop is not installed. Install it from https://altaris.ai/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open Altaris Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
