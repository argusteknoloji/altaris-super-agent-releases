...
export const getMTLSConfig = memoize((): MTLSConfig | undefined => {
  ...
  if (process.env.ALTARIS_CLIENT_CERT) {
    try {
      config.cert = getFsImplementation().readFileSync(
        process.env.ALTARIS_CLIENT_CERT,
        { encoding: 'utf8' },
      )
      logForDebugging(
        'mTLS: Loaded client certificate from ALTARIS_CLIENT_CERT',
      )
    } catch (error) {
  ...
  // Client key
  if (process.env.ALTARIS_CLIENT_KEY) {
    try {
      config.key = getFsImplementation().readFileSync(
        process.env.ALTARIS_CLIENT_KEY,
        { encoding: 'utf8' },
      )
      logForDebugging('mTLS: Loaded client key from ALTARIS_CLIENT_KEY')
    } catch (error) {
  ...
  // Key passphrase
  if (process.env.ALTARIS_CLIENT_KEY_PASSPHRASE) {
    config.passphrase = process.env.ALTARIS_CLIENT_KEY_PASSPHRASE
