 ...
 * validation purposes, so commands like `echo foo > /tmp/altaris/x.txt` don't
 * prompt for permission when /tmp/altaris/ is already in the sandbox allowlist.
 *
 * Respects the deny-within-allow list: paths in denyWithinAllow (like
 * .altaris/settings.json) are still blocked even if their parent is in allowOnly.
 */
...
export function isPathAllowed(
  ...
  // Checks: Windows patterns, Altaris config files, dangerous files (on original + symlink paths)
  if (operationType !== 'read') {
