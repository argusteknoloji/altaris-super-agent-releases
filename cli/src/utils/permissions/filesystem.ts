...
/**
 * If filePath is inside a .altaris/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .altaris/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.altaris', 'skills')),
      prefix: '/.altaris/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.altaris', 'skills')),
      prefix: '~/.altaris/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.altaris/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.altaris/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.altaris${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.altaris${sep}settings.local.json`) ||
    normalizedPath.endsWith(`${sep}.altaris${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.altaris${sep}settings.local.json`)
  ) {
    // Include .altaris/settings.json even for other projects
    return true
  }
...
// Always ask when Altaris tries to edit its own config files
function isClaudeConfigFilePath(filePath: string): boolean {
  if (isClaudeSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .altaris/commands or .altaris/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.altaris', 'commands')
  const agentsDir = join(getOriginalCwd(), '.altaris', 'agents')
  const skillsDir = join(getOriginalCwd(), '.altaris', 'skills')
  const openCommandsDir = join(getOriginalCwd(), '.altaris', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.altaris', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.altaris', 'skills')
 ...
 * Path format: ~/.altaris/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for Altaris to write temporary files.
...
/**
 * Returns the user-specific Altaris temp directory name.
 * On Unix: 'claude-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'altaris' (tmpdir() is already per-user)
 */
...
/**
 * Returns the Altaris temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/claude-{uid}/ (resolved to /private/tmp/claude-{uid}/ on macOS)
 * - On Windows: {tmpdir}/altaris/ (e.g., C:\Users\{user}\AppData\Local\Temp\altaris\)
 * This is a per-user temporary directory used by Altaris for all temp files.
 *
...
// and per-turn from BashTool prompt. Inputs (ALTARIS_TMPDIR env + platform) are
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .altaris/worktrees/ is a structural path (where Altaris stores
      // git worktrees), not a user-created dangerous directory. Skip the .altaris
      // segment when it's followed by 'worktrees'. Any nested .altaris directories
      ...
      if (dir === '.altaris') {
        ...
        if (
          ...
          break // Skip this .altaris, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .altaris , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .altaris , .bashrc..., settings.json.
 ...
 * - Altaris config files (.altaris/settings.json, .altaris/commands/, .altaris/agents/)
 * - MCP CLI state files (managed internally by Altaris)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `Altaris requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for Altaris config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `Altaris requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `Altaris requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .altaris/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `Altaris requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `Altaris requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `Altaris requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `Altaris requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .altaris is a dangerous directory
  ...
  // 1.6. Check for .altaris/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .altaris/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .altaris/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.altaris) rule in userSettings (e.g. from sandbox
  ...
  // "allow Altaris to edit its own settings for this session" option actually works.
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .altaris/ (project or global).
    // Accepts both the broad patterns ('/.altaris/**', '~/.altaris/**') and
    // narrowed ones like '/.altaris/skills/my-skill/**' so users can grant
    ...
    // '/.altaris/../**' from leaking this bypass outside .altaris/.
  ...
  // 1.7. Check comprehensive safety validations (Windows patterns, Altaris config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .altaris/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.altaris/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.altaris/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.altaris is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.altaris/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .altaris/launch.json — desktop preview config (dev server command + port).
  // The desktop's preview_start MCP tool instructs Altaris to create/update
  // this file as part of the preview workflow. Without this carve-out the
  // .altaris/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .altaris/ only (not ~/.altaris/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.altaris', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/altaris/{sanitized-cwd}/)
  ...
  // Tasks directory (~/.altaris/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.altaris/teams/) for swarm coordination
