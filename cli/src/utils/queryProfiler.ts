 ...
 * pipeline from user input to first token arrival. Enable by setting ALTARIS_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.ALTARIS_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set ALTARIS_PROFILE_QUERY=1)'
  }
