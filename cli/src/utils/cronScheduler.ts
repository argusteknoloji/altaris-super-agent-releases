// Non-React scheduler core for .altaris/scheduled_tasks.json.
...
type CronSchedulerOptions = {
  ...
  /**
   * Directory containing .altaris/scheduled_tasks.json. When provided, the
...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while Altaris
  ...
  function check() {
    ...
    // exists to stop two Altaris sessions in the same cwd from double-firing
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while Altaris was not running. ` +
    `${plural ? 'They have' : 'It has'} already been removed from .altaris/scheduled_tasks.json.\n\n` +
