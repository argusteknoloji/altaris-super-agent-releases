...
const RESTORED_KEYS = [
  'ALTARIS_PROVIDER_PROFILE_ENV_APPLIED',
  'ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID',
  'ALTARIS_USE_OPENAI',
  'ALTARIS_USE_GEMINI',
  'ALTARIS_USE_MISTRAL',
  'ALTARIS_USE_GITHUB',
...
describe('applyProviderProfileToProcessEnv', () => {
  test('openai profile clears competing gemini/github flags', async () => {
    ...
    process.env.ALTARIS_USE_GEMINI = '1'
    ...
    expect(process.env.ALTARIS_USE_GEMINI).toBeUndefined()
    expect(process.env.ALTARIS_USE_GITHUB).toBeUndefined()
    expect(String(process.env.ALTARIS_USE_OPENAI)).toBe('1')
    expect(process.env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID).toBe(
      'provider_test',
    )
    expect(getFreshAPIProvider()).toBe('openai')
  })

  test('mistral profile sets ALTARIS_USE_MISTRAL and clears openai flags', async () => {
    const { applyProviderProfileToProcessEnv } =
      await importFreshProviderProfileModules()
    ...
    expect(process.env.ALTARIS_USE_MISTRAL).toBe('1')
    expect(process.env.ALTARIS_USE_OPENAI).toBeUndefined()
    expect(process.env.MISTRAL_MODEL).toBe('devstral-latest')
    expect(getFreshAPIProvider()).toBe('mistral')
  })

  test('gemini profile sets ALTARIS_USE_GEMINI and clears openai flags', async () => {
    const { applyProviderProfileToProcessEnv } =
      await importFreshProviderProfileModules()
    process.env.ALTARIS_USE_OPENAI = '1'
    ...
    expect(process.env.ALTARIS_USE_GEMINI).toBe('1')
    expect(process.env.ALTARIS_USE_OPENAI).toBeUndefined()
...
describe('applyActiveProviderProfileFromConfig', () => {
  ...
  test('applies active profile when a bare ALTARIS_USE_OPENAI flag is stale (no BASE_URL/MODEL)', async () => {
    // Regression: a leftover `ALTARIS_USE_OPENAI=1` in the shell with no
  ...
  test('does not override explicit startup selection when profile marker is stale', async () => {
    const { applyActiveProviderProfileFromConfig } =
      await importFreshProviderProfileModules()
    process.env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED = '1'
    process.env.ALTARIS_USE_OPENAI = '1'
    process.env.OPENAI_BASE_URL = 'http://localhost:11434/v1'
    process.env.OPENAI_MODEL = 'qwen2.5:3b'
  ...
  test('does not re-apply active profile when flags conflict with current provider', async () => {
    ...
    expect(applied).toBeUndefined()
    expect(process.env.ALTARIS_USE_GITHUB).toBe('1')
  ...
  test('applies active profile when no explicit provider is selected', async () => {
    ...
    delete process.env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED
    delete process.env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID
...
describe('setActiveProviderProfile', () => {
  ...
  test('persists no-key openai-compatible profiles for restart fallback', async () => {
    const tempDir = mkdtempSync(join(tmpdir(), 'altaris-provider-'))
    ...
    try {
      ...
      const persisted = JSON.parse(
        readFileSync(join(tempDir, '.altaris-profile.json'), 'utf8'),
      )
...
describe('deleteProviderProfile', () => {
  test('deleting final profile clears provider env when active profile applied it', async () => {
    ...
    expect(process.env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED).toBeUndefined()

    expect(process.env.ALTARIS_USE_OPENAI).toBeUndefined()
    expect(process.env.ALTARIS_USE_GEMINI).toBeUndefined()
    expect(process.env.ALTARIS_USE_GITHUB).toBeUndefined()
    expect(process.env.ALTARIS_USE_BEDROCK).toBeUndefined()
    expect(process.env.ALTARIS_USE_VERTEX).toBeUndefined()
    expect(process.env.ALTARIS_USE_FOUNDRY).toBeUndefined()
