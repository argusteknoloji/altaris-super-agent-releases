...
const TEAMMATE_ENV_VARS = [
  ...
  // its own way via /home/claude/.altaris/remote/.oauth_token regardless;
 ...
 * Always includes CLAUDECODE=1 and ALTARIS_EXPERIMENTAL_AGENT_TEAMS=1,
 * plus any provider/config env vars that are set in the current process.
 */
export function buildInheritedEnvVars(): string {
  const envVars = [
    'CLAUDECODE=1',
    'ALTARIS_EXPERIMENTAL_AGENT_TEAMS=1',
    // Teammates should inherit the leader-selected provider route instead of
    // replaying persisted ~/.altaris or settings.env provider defaults.
    'ALTARIS_PROVIDER_MANAGED_BY_HOST=1',
  ]
