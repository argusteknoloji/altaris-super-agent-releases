// Scheduler lease lock for .altaris/scheduled_tasks.json.
//
// When multiple Altaris sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.altaris', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .altaris/ doesn't exist yet — create it and retry once. In steady
