...
afterEach(() => {
  restoreEnv('ALTARIS_USE_OPENAI', originalEnv.ALTARIS_USE_OPENAI)
  restoreEnv('OPENAI_API_KEY', originalEnv.OPENAI_API_KEY)
  restoreEnv('OPENAI_BASE_URL', originalEnv.OPENAI_BASE_URL)
  restoreEnv('ALTARIS_USE_GEMINI', originalEnv.ALTARIS_USE_GEMINI)
...
test('still errors when no Gemini credential source is available', async () => {
  ...
  await expect(getProviderValidationError(process.env)).resolves.toBe(
    'GEMINI_API_KEY, GOOGLE_API_KEY, GEMINI_ACCESS_TOKEN, or Google ADC credentials are required when ALTARIS_USE_GEMINI=1.',
  )
})
...
test('openai missing key error includes recovery guidance and config locations', async () => {
  ...
  expect(message).toContain(
    'set ALTARIS_USE_OPENAI=0 in your shell environment',
  )
  expect(message).toContain('Saved startup settings can come from')
  expect(message).toContain('.altaris-profile.json')
})
