 ...
 * =====================
 * This module manages an isolated tmux socket for Altaris's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, Altaris could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started Altaris from within tmux.
 *
 * HOW IT WORKS:
 * 1. Altaris creates its own tmux socket: `claude-<PID>` (e.g., `claude-12345`)
 ...
 *
 * This means ANY tmux command run through Altaris - whether via the Tmux tool
 * directly or via Bash - will operate on Altaris's isolated socket, NOT the
 * user's tmux session.
 *
...
const CLAUDE_SOCKET_PREFIX = 'altaris'
...
/**
 * Gets the socket name for Altaris's isolated tmux session.
 * Format: claude-<PID>
 */
...
/**
 * Gets the TMUX environment variable value for Altaris's isolated socket.
 *
 ...
 * the Bash tool will operate on Altaris's socket, NOT the user's session.
 *
...
/**
 * Kills the tmux server for Altaris's isolated socket.
...
async function doInitialize(): Promise<void> {
  ...
  // Pass ALTARIS_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'ALTARIS_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set ALTARIS_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
  ...
  // Any Altaris instance spawned on this socket will inherit this env var,
