...
// Altaris configuration directory names
 ...
 * See: https://github.com/anthropics/altaris/issues/13893
 *
 ...
 * stopping there makes the parent project's `.altaris/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.altaris/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .altaris directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.altaris/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .altaris/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.altaris', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.altaris', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .altaris/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .altaris/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .altaris/<subdir> content — loading the main
    // repo's copy too would duplicate every command/agent/skill
    // (anthropics/altaris#29599, #28182, #26992).
    //
    ...
    // This prevents the same file from appearing multiple times when ~/.altaris is
 ...
 * 3. Can be explicitly enabled via ALTARIS_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
 ...
 * @param dir Directory (eg. "~/.altaris/commands")
 * @returns Array of parsed markdown files with metadata
 */
...
> {
  ...
  // - Fallback: native Node.js (when ALTARIS_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.ALTARIS_USE_NATIVE_FILE_SEARCH)
