...
export function getUserAgent(): string {
  ...
  return `claude-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.ALTARIS_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.ALTARIS_ENTRYPOINT) {
    parts.push(process.env.ALTARIS_ENTRYPOINT)
  }
  ...
  return `altaris/${MACRO.VERSION}${suffix}`
}

// User-Agent for WebFetch requests to arbitrary sites. `Altaris-User` is
// The first-party provider's publicly documented agent for user-initiated fetches (what site
// operators match in robots.txt); the altaris suffix lets them distinguish
// local CLI traffic from altaris.ai server-side fetches.
export function getWebFetchUserAgent(): string {
  const supportUrl =
    getAPIProvider() === 'firstParty'
      ? 'https://support.anthropic.com/'
      : 'https://github.com/argusteknoloji/altaris-super-agent-releases'
  return `Altaris-User (${getClaudeCodeUserAgent()}; +${supportUrl})`
}
