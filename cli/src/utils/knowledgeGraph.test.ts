...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  it('deduplicates entities and updates attributes', () => {
    addGlobalEntity('tool', 'altaris', { status: 'alpha' })
    addGlobalEntity('tool', 'altaris', { status: 'beta', version: '0.6.0' })
    
    const graph = loadProjectGraph(cwd)
    const entities = Object.values(graph.entities).filter(e => e.name === 'altaris')
