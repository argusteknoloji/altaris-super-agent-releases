...
export async function fetchEnvironments(): Promise<EnvironmentResource[]> {
  ...
  if (!accessToken) {
    throw new Error(
      'Altaris web sessions require authentication with a anthropic.com account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
    )
  }
