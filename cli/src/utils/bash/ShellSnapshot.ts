...
/**
 * Generates Altaris specific snapshot content
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Altaris's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and Altaris content
 ...
 *
 * This function is a critical part of Altaris CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows Altaris CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Altaris defaults only`,
        )
      }
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.ALTARIS_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Altaris home: ${getClaudeConfigHomeDir()}`)
