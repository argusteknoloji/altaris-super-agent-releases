...
import { getMaxOutputTokensForModel } from '../services/api/altaris.ts'
import {
...
const originalEnv = {
  ...
  ALTARIS_MAX_OUTPUT_TOKENS: process.env.ALTARIS_MAX_OUTPUT_TOKENS,
  OPENAI_MODEL: process.env.OPENAI_MODEL,
}

beforeEach(() => {
  delete process.env.ALTARIS_USE_OPENAI
  delete process.env.ALTARIS_MAX_OUTPUT_TOKENS
  delete process.env.OPENAI_MODEL
})

afterEach(() => {
  if (originalEnv.ALTARIS_USE_OPENAI === undefined) {
    delete process.env.ALTARIS_USE_OPENAI
  } else {
    process.env.ALTARIS_USE_OPENAI = originalEnv.ALTARIS_USE_OPENAI
  }
  if (originalEnv.ALTARIS_MAX_OUTPUT_TOKENS === undefined) {
    delete process.env.ALTARIS_MAX_OUTPUT_TOKENS
  } else {
    process.env.ALTARIS_MAX_OUTPUT_TOKENS =
      originalEnv.ALTARIS_MAX_OUTPUT_TOKENS
  }
...
test('deepseek-v4-flash clamps oversized max output overrides to the provider limit', () => {
  process.env.ALTARIS_USE_OPENAI = '1'
  process.env.ALTARIS_MAX_OUTPUT_TOKENS = '500000'
  delete process.env.OPENAI_MODEL
...
test('gpt-4o clamps oversized max output overrides to the provider limit', () => {
  process.env.ALTARIS_USE_OPENAI = '1'
  process.env.ALTARIS_MAX_OUTPUT_TOKENS = '32000'
  delete process.env.OPENAI_MODEL
...
test('gpt-5.4 family keeps large max output overrides within provider limits', () => {
  process.env.ALTARIS_USE_OPENAI = '1'
  process.env.ALTARIS_MAX_OUTPUT_TOKENS = '200000'
...
test('DashScope models clamp oversized max output overrides to the provider limit', () => {
  process.env.ALTARIS_USE_OPENAI = '1'
  process.env.ALTARIS_MAX_OUTPUT_TOKENS = '100000'
