...
export function getAgentModel(
  ...
  if (process.env.ALTARIS_SUBAGENT_MODEL) {
    return parseUserSpecifiedModel(process.env.ALTARIS_SUBAGENT_MODEL)
  }
  ...
  // Altaris-native providers (Bedrock, Vertex, Foundry, official Anthropic API)
  ...
  // For haiku/sonnet aliases on non-Altaris-native providers, inherit parent model.
  ...
  if (
    ...
    // Non-Altaris-native provider → inherit parent model
 ...
 * See https://github.com/anthropics/altaris/issues/30815.
 *
...
/**
 * Check if the current provider is Altaris-native (has guaranteed haiku/sonnet models).
 * Altaris-native providers: Bedrock, Vertex, Foundry, official Anthropic API.
 * Non-Altaris-native: OpenAI, Gemini, Mistral, GitHub, NVIDIA NIM, MiniMax,
