...
async function importFreshModelModule() {
  ...
  mock.module('./providers.js', () => ({
    getAPIProvider: () => {
      ...
      if (process.env.ALTARIS_USE_GEMINI) return 'gemini'
      if (process.env.ALTARIS_USE_MISTRAL) return 'mistral'
      if (process.env.ALTARIS_USE_GITHUB) return 'github'
      if (process.env.ALTARIS_USE_OPENAI) {
      ...
      if (process.env.ALTARIS_USE_BEDROCK) return 'bedrock'
      if (process.env.ALTARIS_USE_VERTEX) return 'vertex'
      if (process.env.ALTARIS_USE_FOUNDRY) return 'foundry'
      return 'firstParty'
    },
  }))
