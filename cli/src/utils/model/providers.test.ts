...
test.each([
  ['ALTARIS_USE_OPENAI', 'openai'],
  ['ALTARIS_USE_GITHUB', 'github'],
  ['ALTARIS_USE_GEMINI', 'gemini'],
  ['ALTARIS_USE_BEDROCK', 'bedrock'],
  ['ALTARIS_USE_VERTEX', 'vertex'],
  ['ALTARIS_USE_FOUNDRY', 'foundry'],
] as const)(
...
test('isGithubNativeAnthropicMode: false when ALTARIS_USE_GITHUB is not set', async () => {
  clearProviderEnv()
...
test('isGithubNativeAnthropicMode: false for non-Altaris model', async () => {
  clearProviderEnv()
  process.env.ALTARIS_USE_GITHUB = '1'
