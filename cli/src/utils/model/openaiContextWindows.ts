 ...
 *
 * When ALTARIS_USE_OPENAI=1, getContextWindowForModel() falls through to
...
const OPENAI_CONTEXT_WINDOWS: Record<string, number> = {
  ...
  // LiteLLM format — when Altaris talks to a LiteLLM proxy, Copilot models
  ...
  // NOTE: bare Altaris model names (e.g. 'claude-sonnet-4') are intentionally
  ...
  // Google (native via ALTARIS_USE_GEMINI)
...
const OPENAI_MAX_OUTPUT_TOKENS: Record<string, number> = {
  ...
  // NOTE: bare Altaris model names omitted — see context windows comment above.
...
// Set ALTARIS_OPENAI_CONTEXT_WINDOWS to a JSON object mapping model name
// → context-window token count to add or override entries without editing
// this file.  Example:
//   ALTARIS_OPENAI_CONTEXT_WINDOWS='{"my-corp/llm-v2":200000}'
const OPENAI_EXTERNAL_CONTEXT_WINDOWS: Record<string, number> = (() => {
  try {
    const raw = process.env.ALTARIS_OPENAI_CONTEXT_WINDOWS
    if (raw) {
...
// Set ALTARIS_OPENAI_MAX_OUTPUT_TOKENS to a JSON object mapping model name
// → max output token count.
const OPENAI_EXTERNAL_MAX_OUTPUT_TOKENS: Record<string, number> = (() => {
  try {
    const raw = process.env.ALTARIS_OPENAI_MAX_OUTPUT_TOKENS
    if (raw) {
      const parsed = JSON.parse(raw)
      if (typeof parsed === 'object' && parsed !== null) return parsed as Record<string, number>
    }
