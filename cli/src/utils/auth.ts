...
/**
 * CCR and Altaris Desktop spawn the CLI with OAuth and should never fall back
 * to the user's ~/.altaris/settings.json API-key config (apiKeyHelper,
 ...
 * who runs `altaris` in their terminal with an API key sees every CCD session
 * also use that key — and fail if it's stale/wrong-org.
 */
function isManagedOAuthContext(): boolean {
  return (
    isEnvTruthy(process.env.ALTARIS_REMOTE) ||
    process.env.ALTARIS_ENTRYPOINT === 'claude-desktop'
  )
}
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // `altaris ssh` remote: ANTHROPIC_UNIX_SOCKET tunnels API calls through a
  // local auth-injecting proxy. The launcher sets ALTARIS_OAUTH_TOKEN as a
  // placeholder iff the local side is a subscriber (so the remote includes the
  // oauth-2025 beta header to match what the proxy will inject). The remote's
  // ~/.altaris settings (apiKeyHelper, settings.env.ANTHROPIC_API_KEY) MUST NOT
  ...
  if (process.env.ANTHROPIC_UNIX_SOCKET) {
    return !!process.env.ALTARIS_OAUTH_TOKEN
  }

  const is3P =
    isEnvTruthy(process.env.ALTARIS_USE_BEDROCK) ||
    isEnvTruthy(process.env.ALTARIS_USE_VERTEX) ||
    isEnvTruthy(process.env.ALTARIS_USE_FOUNDRY) ||
    isEnvTruthy(process.env.ALTARIS_USE_OPENAI) ||
    isEnvTruthy(process.env.ALTARIS_USE_GEMINI) ||
    isEnvTruthy(process.env.ALTARIS_USE_MISTRAL) ||
  ...
  const hasExternalAuthToken =
    ...
    apiKeyHelper ||
    process.env.ALTARIS_API_KEY_FILE_DESCRIPTOR
...
export function getAuthTokenSource() {
  ...
  if (process.env.ALTARIS_OAUTH_TOKEN) {
    return { source: 'ALTARIS_OAUTH_TOKEN' as const, hasToken: true }
  }
  ...
  if (oauthTokenFromFd) {
    ...
    if (process.env.ALTARIS_OAUTH_TOKEN_FILE_DESCRIPTOR) {
      return {
        source: 'ALTARIS_OAUTH_TOKEN_FILE_DESCRIPTOR' as const,
        hasToken: true,
      }
    }
    return {
  ...
  if (shouldUseClaudeAIAuth(oauthTokens?.scopes) && oauthTokens?.accessToken) {
    return { source: 'altaris.ai' as const, hasToken: true }
  }
...
export function getAnthropicApiKeyWithSource(
  ...
  // Always check for direct environment variable when the user ran altaris --print.
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      ...
      !process.env.ALTARIS_OAUTH_TOKEN_FILE_DESCRIPTOR
    ) {
      throw new Error(
        'ANTHROPIC_API_KEY or ALTARIS_OAUTH_TOKEN env var is required',
      )
    }
 ...
 * from ~/.altaris/settings.json or project settings is ignored.
 */
 ...
 * Uses ALTARIS_API_KEY_HELPER_TTL_MS env var if set and valid,
 * otherwise defaults to 5 minutes
 */
export function calculateApiKeyHelperTTL(): number {
  const envTtl = process.env.ALTARIS_API_KEY_HELPER_TTL_MS
  ...
  if (envTtl) {
    ...
    logForDebugging(
      `Found ALTARIS_API_KEY_HELPER_TTL_MS env var, but it was not a valid number. Got ${envTtl}`,
      { level: 'error' },
    )
  }
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.altaris.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.altaris.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.altaris.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  if (process.env.ALTARIS_OAUTH_TOKEN) {
    ...
    return {
      accessToken: process.env.ALTARIS_OAUTH_TOKEN,
      refreshToken: null,
      expiresAt: null,
...
// In-flight dedup: when N altaris.ai proxy connectors hit 401 with the same
...
async function checkAndRefreshOAuthTokenIfNeededImpl(
  ...
  try {
    ...
    const refreshedTokens = await refreshOAuthToken(lockedTokens.refreshToken, {
      // For anthropic.com subscribers, omit scopes so the default
...
export function is1PApiCustomer(): boolean {
  ...
  // 1. anthropic.com subscribers (Max, Pro, Enterprise, Team)
  // 2. Vertex AI users
  // 3. AWS Bedrock users
  // 4. Foundry users
  ...
  // Exclude anthropic.com subscribers
...
export function isOverageProvisioningAllowed(): boolean {
  ...
  // Must be a Altaris subscriber with a supported subscription type
...
export function getOtelHeadersFromHelper(): Record<string, string> {
  ...
  const debounceMs = parseInt(
    process.env.ALTARIS_OTEL_HEADERS_HELPER_DEBOUNCE_MS ||
...
export function getAccountInformation() {
  ...
  if (
    authTokenSource === 'ALTARIS_OAUTH_TOKEN' ||
    authTokenSource === 'ALTARIS_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ) {
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  // `altaris ssh` remote: real auth lives on the local machine and is injected
  ...
  // in ~/.altaris.json is user-writable and cannot be trusted.
  const { source } = getAuthTokenSource()
  const isEnvVarToken =
    source === 'ALTARIS_OAUTH_TOKEN' ||
    source === 'ALTARIS_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `verification (tokens from 'altaris setup-token' do not include this scope).\n` +
        `Try again, or obtain a full-scope token via 'altaris auth login'.`,
    }
  }
  ...
  if (isEnvVarToken) {
    const envVarName =
      source === 'ALTARIS_OAUTH_TOKEN'
        ? 'ALTARIS_OAUTH_TOKEN'
        : 'ALTARIS_OAUTH_TOKEN_FILE_DESCRIPTOR'
    return {
      valid: false,
      message:
  ...
  return {
    ...
    message:
      ...
      `Please log in with the correct organization: altaris auth login`,
  }
}
