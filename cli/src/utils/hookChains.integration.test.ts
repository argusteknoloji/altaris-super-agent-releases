...
const originalHookChainsEnabled = process.env.ALTARIS_ENABLE_HOOK_CHAINS

async function createConfigFile(config: unknown): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'altaris-hook-chains-int-'))
  tempDirs.push(dir)
...
beforeEach(() => {
  process.env.ALTARIS_ENABLE_HOOK_CHAINS = '1'
})
...
afterEach(async () => {
  ...
  if (originalHookChainsEnabled === undefined) {
    delete process.env.ALTARIS_ENABLE_HOOK_CHAINS
  } else {
    process.env.ALTARIS_ENABLE_HOOK_CHAINS = originalHookChainsEnabled
  }
