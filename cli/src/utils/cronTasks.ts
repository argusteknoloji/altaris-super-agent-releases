// Scheduled prompts, stored in <project>/.altaris/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.altaris', 'scheduled_tasks.json')
...
/**
 * Read and parse .altaris/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .altaris/scheduled_tasks.json with the given tasks. Creates .altaris/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.altaris'), { recursive: true })
 ...
 * written to .altaris/scheduled_tasks.json and dies with the process. The
 ...
 * and recurring tasks — a recurring task whose window passed while Altaris
