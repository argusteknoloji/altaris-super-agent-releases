 ...
 * (e.g. anthropics/altaris, anthropic-experimental/sandbox-runtime).
...
export function sanitizeModelName(shortName: string): string {
  ...
  if (shortName.includes('sonnet-4-6')) return 'sonnet'
...
/**
 * Attribution state for tracking Altaris's contributions to files.
 */
...
/**
 * Summary of Altaris's contribution for a commit.
 */
...
export function getClientSurface(): string {
  return process.env.ALTARIS_ENTRYPOINT ?? 'cli'
}
...
function computeFileModificationState(
  ...
  try {
    // Calculate Altaris's character contribution
...
/**
 * Track a file modification by Altaris.
...
/**
 * Track a file creation by Altaris (e.g., via bash command).
 * Used when Altaris creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by Altaris (e.g., via bash rm command).
 * Used when Altaris deletes a file through a non-tracked mechanism.
 */
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // Altaris deleted this file (tracked deletion)
        ...
        try {
          ...
          if (fileState) {
            ...
            // New file not created by Altaris
