...
const missingCodexAuthPath = join(tmpdir(), 'altaris-missing-codex-auth.json')
...
test('matching persisted gemini env is reused for gemini launch', async () => {
  ...
  assert.equal(env.ALTARIS_USE_GEMINI, '1')
  assert.equal(env.ALTARIS_USE_OPENAI, undefined)
...
test('openai env variables take precedence over gemini', async () => {
  ...
  assert.equal(env.ALTARIS_USE_GEMINI, undefined) 
  assert.equal(env.ALTARIS_USE_OPENAI, '1')
...
test('codex launch prefers auth account id over stale persisted value', async () => {
  const codexHome = mkdtempSync(join(tmpdir(), 'altaris-codex-'))
  try {
    writeFileSync(
...
test('saveProfileFile writes a profile that loadProfileFile can read back', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'altaris-profile-file-'))
...
test('clearPersistedCodexOAuthProfile removes only persisted Codex OAuth profiles', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'altaris-codex-oauth-profile-'))
...
test('buildStartupEnvFromProfile preserves explicit GitHub provider settings when the legacy file is stale', async () => {
  ...
  assert.equal(env.ALTARIS_USE_GITHUB, '1')
...
test('applySavedProfileToCurrentSession can switch away from GitHub provider env', async () => {
  ...
  assert.equal(processEnv.ALTARIS_USE_GITHUB, undefined)
  assert.equal(processEnv.ALTARIS_USE_OPENAI, '1')
...
test('buildStartupEnvFromProfile preserves plural-profile env when the legacy file is stale', async () => {
  ...
  // addProviderProfile does NOT sync the legacy .altaris-profile.json,
  ...
  // env with ALTARIS_PROVIDER_PROFILE_ENV_APPLIED=1. The legacy-file
  // load must NOT overwrite that env — it previously did, surfacing as
  // "banner shows the wrong provider / model".
  const processEnv = {
    ALTARIS_PROVIDER_PROFILE_ENV_APPLIED: '1',
    ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID: 'saved_moonshot',
    ALTARIS_USE_OPENAI: '1',
  ...
  assert.equal(env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED, '1')
  assert.equal(env.ALTARIS_PROVIDER_PROFILE_ENV_APPLIED_ID, 'saved_moonshot')
})
