...
export function PreflightStep(t0) {
  ...
  if ($[6] !== isChecking || $[7] !== result || $[8] !== showSpinner) {
    t5 = isChecking && showSpinner ? <Box paddingLeft={1}><Spinner /><Text>Checking connectivity...</Text></Box> : !result?.success && !isChecking && <Box flexDirection="column" gap={1}><Text color="error">Altaris bulut servislerine bağlanılamadı</Text><Text color="error">{result?.error}</Text><Box flexDirection="column" gap={1}><Text>İnternet bağlantınızı ve ağ ayarlarınızı kontrol edin.</Text></Box></Box>;
    $[6] = isChecking;
    $[7] = result;
    $[8] = showSpinner;
    $[9] = t5;
