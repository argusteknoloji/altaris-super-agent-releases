/**
 * Perfetto Tracing for Altaris (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via ALTARIS_PERFETTO_TRACE=1 or ALTARIS_PERFETTO_TRACE=<path>
 * 2. Optionally set ALTARIS_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Altaris normally
 * 4. Trace file is written to ~/.altaris/traces/trace-<session-id>.json
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.ALTARIS_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if ALTARIS_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.ALTARIS_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
