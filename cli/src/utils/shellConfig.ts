 ...
 * Used for managing altaris aliases and PATH entries
 */
...
export const CLAUDE_ALIAS_REGEX = /^\s*alias\s+altaris\s*=/
...
/**
 * Filter out installer-created altaris aliases from an array of lines
 * Only removes aliases pointing to $HOME/.altaris/local/altaris
...
/**
 * Check if a altaris alias exists in any shell config file
  ...
  for (const configPath of Object.values(configs)) {
    ...
    for (const line of lines) {
      if (CLAUDE_ALIAS_REGEX.test(line)) {
        ...
        const match = line.match(/alias\s+altaris=["']?([^"'\s]+)/)
        if (match && match[1]) {
          return match[1]
        }
      }
...
/**
 * Check if a altaris alias exists and points to a valid executable
