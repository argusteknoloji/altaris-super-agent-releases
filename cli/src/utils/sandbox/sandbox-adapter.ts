/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with Altaris CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and Altaris CLI's
...
/**
 * Resolve Altaris-specific path patterns for sandbox-runtime.
 *
 * Altaris uses special path prefixes in permission rules:
...
/**
 * Convert Altaris settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and Altaris temp directory as writable
  ...
  // This blocks settings in the original working directory (where Altaris started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.altaris', 'settings.json'))
    denyWrite.push(resolve(cwd, '.altaris', 'settings.local.json'))
  }

  // Block writes to .altaris/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .altaris/commands and
  // .altaris/agents but not .altaris/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full Altaris capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.altaris', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.altaris', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when Altaris's unsandboxed git runs.
  //
...
// scrubbed if they appear after a sandboxed command. See anthropics/altaris#29316.
const bareGitRepoScrubPaths: string[] = []

/**
 * Delete bare-repo files planted at cwd during a sandboxed command, before
 * Altaris's unsandboxed git calls can see them. See the SECURITY block above
 * bareGitRepoFiles. anthropics/altaris#29316.
 */
 ...
 * otherwise disable the sandbox via .altaris/settings.json.
 */
 ...
 * This is a Altaris CLI-specific function that updates local settings.
 */
...
/**
 * Altaris CLI sandbox manager - wraps sandbox-runtime with Altaris-specific features
 */
