...
export async function getClaudeDesktopConfigPath(): Promise<string> {
  ...
  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - Altaris Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  if (windowsHome) {
    ...
    const configPath = `/mnt/c${wslPath}/AppData/Roaming/Altaris/claude_desktop_config.json`
  ...
  try {
    ...
    try {
      ...
      // Look for Altaris Desktop config in each user directory
  ...
  throw new Error(
    'Could not find Altaris Desktop config file in Windows. Make sure Altaris Desktop is installed on Windows.',
  )
}
...
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - Altaris Desktop integration only works on macOS and WSL.',
    )
  }
  try {
