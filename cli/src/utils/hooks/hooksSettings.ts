...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.altaris/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.altaris/settings.json)'
    case 'projectSettings':
      return 'Project settings (.altaris/settings.json)'
    case 'localSettings':
      return 'Local settings (.altaris/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.altaris/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.altaris/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by Altaris)'
    default:
