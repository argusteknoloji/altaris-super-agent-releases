...
function getCliBinaryName(): string {
  return MACRO.PACKAGE_URL === '@anthropic-ai/altaris'
    ? 'altaris'
    : 'altaris'
}
...
> {
  ...
  const packagesToCheck = ['@anthropic-ai/altaris']
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    // First check for active installations via bin/altaris
    // Linux / macOS have prefix/bin/altaris and prefix/lib/node_modules
    // Windows has prefix/altaris and prefix/node_modules
    ...
    if (globalBinExists) {
      ...
      // When npm is installed via Homebrew, both can exist at /opt/homebrew/bin/altaris
      ...
      // If no bin/altaris exists, check for orphaned packages (no bin/altaris symlink)
...
async function detectConfigurationIssues(
  ...
  if (type === 'npm-local') {
    // Check if altaris is already accessible via PATH
    const whichResult = await which(getCliBinaryName())
    const claudeInPath = !!whichResult

    // Only show warning if altaris is NOT in PATH AND no valid alias exists
    if (!claudeInPath && !validAlias) {
      if (existingAlias) {
        // Alias exists but points to invalid target
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Alias exists but points to invalid target: ${existingAlias}. Update alias: alias ${getCliBinaryName()}="~/.altaris/local/${getCliBinaryName()}"`,
        })
      } else {
        // No alias exists and not in PATH
        warnings.push({
          issue: 'Local installation not accessible',
          fix: `Create alias: alias ${getCliBinaryName()}="~/.altaris/local/${getCliBinaryName()}"`,
        })
      }
    }
  }
  ...
  if (installationType === 'native') {
    ...
    for (const install of npmInstalls) {
      if (install.type === 'npm-global') {
        let uninstallCmd = 'npm -g uninstall @anthropic-ai/altaris'
        if (
          MACRO.PACKAGE_URL &&
          MACRO.PACKAGE_URL !== '@anthropic-ai/altaris'
        ) {
