...
function getLegacyLocalInstallDir(homeDir = homedir()): string {
  return join(homeDir, '.altaris', 'local')
}
...
function getCandidateLocalBinaryPaths(localInstallDir: string): string[] {
  return [
    join(localInstallDir, 'node_modules', '.bin', 'altaris'),
    join(localInstallDir, 'node_modules', '.bin', 'altaris'),
  ]
}
...
export function isManagedLocalInstallationPath(execPath: string): boolean {
  ...
  return (
    normalizedExecPath.includes('/.altaris/local/node_modules/') ||
    normalizedExecPath.includes('/.altaris/local/node_modules/')
  )
}

export function getLocalClaudePath(): string {
  return join(getLocalInstallDir(), 'altaris')
}
...
export async function ensureLocalPackageEnvironment(): Promise<boolean> {
  try {
    ...
    await writeIfMissing(
      ...
      jsonStringify(
        { name: 'altaris-local', version: '0.0.1', private: true },
        null,
        2,
      ),
    )
    ...
    const created = await writeIfMissing(
      wrapperPath,
      `#!/bin/sh\nexec "${localInstallDir}/node_modules/.bin/altaris" "$@"`,
      0o755,
    )
    if (created) {
...
/**
 * Install or update Altaris CLI package in the local directory
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install Altaris CLI package: ${result.stderr}`,
      )
      logError(error)
