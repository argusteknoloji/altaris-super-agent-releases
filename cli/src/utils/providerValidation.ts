...
function getOpenAIMissingKeyMessage(): string {
  ...
  return [
    'OPENAI_API_KEY is required when ALTARIS_USE_OPENAI=1 and OPENAI_BASE_URL is not local.',
    `To recover, run /provider and switch provider, or set ALTARIS_USE_OPENAI=0 in your shell environment.`,
...
export async function getProviderValidationError(
  ...
  const useOpenAI = isEnvTruthy(env.ALTARIS_USE_OPENAI)
  const useGithub = isEnvTruthy(env.ALTARIS_USE_GITHUB)

  if (isEnvTruthy(env.ALTARIS_USE_GEMINI)) {
    const geminiCredential = await (
      options?.resolveGeminiCredential ?? resolveGeminiCredential
    )(env)
    if (geminiCredential.kind === 'none') {
      return 'GEMINI_API_KEY, GOOGLE_API_KEY, GEMINI_ACCESS_TOKEN, or Google ADC credentials are required when ALTARIS_USE_GEMINI=1.'
    }
    return null
  }
...
export async function validateProviderEnvForStartupOrExit(
  ...
  console.error(
    `Warning: provider configuration is incomplete.\n${error}\nAltaris will continue starting so you can run /provider and repair the saved provider settings.`,
  )
}
