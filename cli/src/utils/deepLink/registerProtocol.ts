...
export const MACOS_BUNDLE_ID = 'com.anthropic.altaris-url-handler'
const APP_NAME = 'Altaris URL Handler'
const DESKTOP_FILE_NAME = 'altaris-url-handler.desktop'
const MACOS_APP_NAME = 'Altaris URL Handler.app'
 ...
 * already-installed (and signed) `altaris` binary. When macOS opens a
 * `claude-cli://` URL, it launches `altaris` through this app bundle.
 * Altaris then uses the url-handler NAPI module to read the URL from the
...
async function registerMacos(claudePath: string): Promise<void> {
  ...
  // Info.plist — registers the URL scheme with altaris as the executable
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>Altaris Deep Link</string>
  ...
  // Symlink to the already-signed altaris binary — avoids a new executable
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for Altaris
 ...
 * After registration, clicking a `claude-cli://` link will invoke altaris.
 */
...
/**
 * Resolve the altaris binary path for protocol registration. Prefers the
 * native installer's stable symlink (~/.local/bin/altaris) which survives
...
async function resolveClaudePath(): Promise<string> {
  const binaryName = process.platform === 'win32' ? 'altaris.exe' : 'altaris'
 ...
 * points at the expected `altaris` binary. Reads the registration artifact
 * directly (symlink target, .desktop Exec line, registry value) rather than
 * a cached flag in ~/.altaris.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.altaris (per-machine, not synced) rather than ~/.altaris.json (can sync).
