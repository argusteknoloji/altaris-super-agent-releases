 ...
 * Protocol Handler
 *
 * Entry point for `altaris --handle-uri <url>`. When the OS invokes altaris
 * with a `claude-cli://` URL, this module:
 *   1. Parses the URI into a structured action
 *   2. Detects the user's terminal emulator
 *   3. Opens a new terminal window running altaris with the appropriate args
 *
 ...
 * This function parses the URI, resolves the altaris binary, and
...
export async function handleDeepLinkUri(uri: string): Promise<number> {
  ...
  // baked at registration time, and we want the terminal-launched Altaris to
...
/**
 * Handle the case where altaris was launched as the app bundle's executable
...
/**
 * Resolve the working directory for the launched Altaris instance.
 * Precedence: explicit cwd > repo lookup (MRU clone) > home.
 * A repo that isn't cloned locally is not an error — fall through to home
 * so a web link referencing a repo the user doesn't have still opens Altaris.
 *
