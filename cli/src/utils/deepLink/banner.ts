 ...
 * notice which directory — and therefore which ALTARIS.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which ALTARIS.md
 ...
 * and whether its ALTARIS.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — ALTARIS.md may be stale' : ''}`,
    )
  }
