 ...
 * user-controlled files (~/.altaris/settings.json and ~/.altaris.json),
 ...
 *
 * We read from global config (~/.altaris.json) and user settings
 * (~/.altaris/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.altaris/settings.json),
