...
export function getPlanModeV2AgentCount(): number {
  ...
  if (process.env.ALTARIS_PLAN_V2_AGENT_COUNT) {
    const count = parseInt(process.env.ALTARIS_PLAN_V2_AGENT_COUNT, 10)
...
export function getPlanModeV2ExploreAgentCount(): number {
  if (process.env.ALTARIS_PLAN_V2_EXPLORE_AGENT_COUNT) {
    const count = parseInt(
      process.env.ALTARIS_PLAN_V2_EXPLORE_AGENT_COUNT,
      10,
    )
    if (!isNaN(count) && count > 0 && count <= 10) {
      return count
...
export function isPlanModeInterviewPhaseEnabled(): boolean {
  const env = process.env.ALTARIS_PLAN_MODE_INTERVIEW_PHASE
