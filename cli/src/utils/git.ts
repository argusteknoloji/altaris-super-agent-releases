...
const resolveCanonicalRoot = memoizeWithLRU(
  (gitRoot: string): string => {
    try {
      ...
      // dialog and executing hooks from .altaris/settings.json on startup.
      //
      ...
      // Use the common dir itself as the stable identity (anthropics/altaris#27994).
...
export const stashToCleanState = async (message?: string): Promise<boolean> => {
  try {
    const stashMessage =
      message || `Altaris auto-stash - ${new Date().toISOString()}`
