...
import { getExternalClaudeMdIncludes, getMemoryFiles, shouldShowClaudeMdExternalIncludesWarning } from './utils/altarismd.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], altarisInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  // and checks ALTARIS.md external includes. bypassPermissions mode
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    if (usesAnthropicSetup) {
      ...
      // Check for altaris.md includes that need approval
  ...
  // processes but ignored by Altaris itself (see auth.ts).
  ...
  // Show Chrome onboarding for first-time Altaris Browser Bridge users
  if (altarisInChrome && !getGlobalConfig().hasCompletedClaudeInChromeOnboarding) {
    const {
...
export function getRenderContext(exitOnCtrlC: boolean): {
  ...
  const frameTimingLogPath = process.env.ALTARIS_FRAME_TIMING_LOG;
  return {
