...
import { clearMemoryFileCaches } from './utils/altarismd.js'
...
export async function setup(
  ...
  if (!nodeVersion || parseInt(nodeVersion) < 18) {
    ...
    console.error(
      chalk.bold.red(
        'Error: Altaris requires Node.js version 18 or higher.',
      ),
    )
    process.exit(1)
  }
  ...
  if (!isBareMode() || messagingSocketPath !== undefined) {
    ...
    // and $ALTARIS_MESSAGING_SOCKET is exported before any hook
  ...
  if (worktreeEnabled) {
    ...
    // .altaris/settings.json. Re-read from the worktree and re-capture hooks.
    updateHooksConfigSnapshot()
  }
  ...
  // When ALTARIS_SYNC_PLUGIN_INSTALL is set, skip all plugin prefetch.
  ...
  const skipPluginPrefetch =
    (getIsNonInteractiveSession() &&
      isEnvTruthy(process.env.ALTARIS_SYNC_PLUGIN_INSTALL)) ||
  ...
  if (
    ...
    if (
      ...
      !isEnvTruthy(process.env.ALTARIS_BUBBLEWRAP)
    ) {
      // biome-ignore lint/suspicious/noConsole:: intentional console output
      console.error(
