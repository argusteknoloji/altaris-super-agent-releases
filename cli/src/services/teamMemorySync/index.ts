 ...
 *   GET  /api/altaris/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/altaris/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/altaris/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
// GB-tunable per-org (altaris_team_memory_limits), so any compile-time
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/altaris/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // altaris_team_memory_limits; we have no way to know it in advance.
...
export async function pullTeamMemory(
  ...
  if (filesWritten > 0) {
    const { clearMemoryFileCaches } = await import('../../utils/altarismd.js')
