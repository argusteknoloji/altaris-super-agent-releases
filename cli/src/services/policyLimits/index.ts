 ...
 * - OAuth users (anthropic.com): Only Team and Enterprise/C4E subscribers are eligible
...
function getPolicyLimitsEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/altaris/policy_limits`
}
...
export function isPolicyLimitsEligible(): boolean {
  ...
  // For OAuth users, check if they have anthropic.com tokens
  const tokens = getClaudeAIOAuthTokens()
  if (!tokens?.accessToken) {
    return false
  }

  // Must have anthropic.com inference scope
...
function getAuthHeaders(): {
  ...
  // Fall back to OAuth tokens (for anthropic.com users)
