...
// OAuth credentials as Altaris.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as Altaris — available when the
  // user is authenticated with Anthropic (anthropic.com subscriber or has
  // valid OAuth tokens).
...
export async function connectVoiceStream(
  ...
  // visibility.external: true). We target that host instead of altaris.ai
  // because the altaris.ai CF zone uses TLS fingerprinting and challenges
  // non-browser clients (anthropics/altaris#34094). Same private-api
  // pod, same OAuth Bearer auth — just a CF zone that doesn't block us.
  // Desktop dictation still uses altaris.ai (Swift URLSession has a
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (anthropics/altaris#40510).
