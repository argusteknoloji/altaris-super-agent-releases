...
afterEach(() => {
  ...
  if (originalEnv.ALTARIS_USE_GITHUB === undefined) delete process.env.ALTARIS_USE_GITHUB
  else process.env.ALTARIS_USE_GITHUB = originalEnv.ALTARIS_USE_GITHUB
...
function createTempAuthJson(payload: Record<string, unknown>): string {
  const dir = mkdtempSync(join(tmpdir(), 'altaris-codex-'))
  tempDirs.push(dir)
...
describe('Codex request translation', () => {
  ...
  test('recovers Codex web search text and sources from sparse completed response', () => {
        ...
        output: [
          {
            ...
            sources: [
              {
                title: 'Altaris repo',
                url: 'https://github.com/example/altaris',
              },
            ],
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Altaris is available on GitHub.',
                sources: [
                  {
                    title: 'Docs',
                    url: 'https://docs.example.com/altaris',
                  },
                ],
              },
            ],
    ...
    expect(output.results).toEqual([
      'Altaris is available on GitHub.',
      {
            ...
            content: [
              {
                ...
                sources: [
                  { title: 'Docs', url: 'https://docs.example.com/altaris' },
                ],
              },
            ],
          },
