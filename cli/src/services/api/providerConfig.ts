...
export function resolveProviderRequest(options?: {
  ...
  const isGithubMode = isEnvTruthy(process.env.ALTARIS_USE_GITHUB)
  const isMistralMode = isEnvTruthy(process.env.ALTARIS_USE_MISTRAL)
  const isGeminiMode = isEnvTruthy(process.env.ALTARIS_USE_GEMINI)
...
export function getAdditionalModelOptionsCacheScope(): string | null {
  if (!isEnvTruthy(process.env.ALTARIS_USE_OPENAI)) {
