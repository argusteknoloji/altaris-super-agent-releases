    ...
    try {
      const response = await withOAuth401Retry(() => {
        ...
        return axios.get<GroveConfig>(
          `${getOauthConfig().BASE_API_URL}/api/altaris_grove`,
          {
            headers: {
              ...authHeaders.headers,
...
export function calculateShouldShowGrove(
  ...
  // whether to help improve Altaris.
...
export async function checkGroveForNonInteractive(): Promise<void> {
  ...
  if (shouldShowGrove) {
    ...
    if (config === null || config.notice_is_grace_period) {
      ...
      writeToStderr(
        '\nAn update to our Consumer Terms and Privacy Policy will take effect on October 8, 2025. Run `altaris` to review the updated terms.\n\n',
      )
      await markGroveNoticeViewed()
    } else {
      // Grace period has ended - show error message and exit
      writeToStderr(
        '\n[ACTION REQUIRED] An update to our Consumer Terms and Privacy Policy has taken effect on October 8, 2025. You must run `altaris` to review the updated terms.\n\n',
      )
