...
const originalUseGithub = process.env.ALTARIS_USE_GITHUB

afterEach(() => {
  if (originalUseGithub === undefined) {
    delete process.env.ALTARIS_USE_GITHUB
  } else {
    process.env.ALTARIS_USE_GITHUB = originalUseGithub
  }
})
...
test('resolveProviderRequest applies GitHub normalization when ALTARIS_USE_GITHUB=1', () => {
