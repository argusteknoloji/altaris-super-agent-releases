...
describe('extractCacheMetrics — Copilot / Ollama (unsupported)', () => {
  ...
  test('Copilot serving Altaris (copilot-altaris) is supported and uses Anthropic fields', () => {
    const usage = {
      input_tokens: 200,
      cache_read_input_tokens: 800,
      cache_creation_input_tokens: 100,
    }
    const m = extractCacheMetrics(usage, 'copilot-altaris')
    expect(m.supported).toBe(true)
    expect(m.read).toBe(800)
...
describe('resolveCacheProvider', () => {
  ...
  test('github without altaris hint → copilot (unsupported)', () => {
    expect(resolveCacheProvider('github')).toBe('copilot')
  })
  test('github with altaris hint → copilot-altaris', () => {
    expect(
      resolveCacheProvider('github', { githubNativeAnthropic: true }),
    ).toBe('copilot-altaris')
  })
