...
/**
 * Fetch the user's first Altaris token date and store in config.
 * This is called after successful login to cache when they started using Altaris.
 */
export async function fetchAndStoreClaudeCodeFirstTokenDate(): Promise<void> {
  try {
    ...
    const url = `${oauthConfig.BASE_API_URL}/api/organization/altaris_first_token_date`
