 ...
 *   - Honest N/A: Copilot (non-Altaris) and Ollama do not expose cache data
 ...
 *   - Copilot (non-Altaris) / Ollama: not reported → supported=false
 */
 ...
 * `copilot-altaris` (Anthropic shim via Copilot with explicit cache)
...
export function resolveCacheProvider(
  ...
  if (provider === 'github') {
    return hints?.githubNativeAnthropic ? 'copilot-altaris' : 'copilot'
  }
...
export function extractCacheMetrics(
  ...
  // Copilot vanilla (no Altaris) and Ollama don't expose cache fields at
  // all as a provider-identity matter. These are explicit provider
  // selections (via ALTARIS_USE_GITHUB and the Ollama base-URL
