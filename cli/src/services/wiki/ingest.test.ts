...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'altaris-wiki-ingest-'))
  tempDirs.push(dir)
  return dir
}
...
test('ingestLocalWikiSource creates a source note and updates log/index', async () => {
  ...
  expect(result.sourceNote.startsWith('.altaris/wiki/sources/')).toBe(true)
  ...
  expect(index).toContain(result.sourceNote.replace('.altaris/wiki/', './'))
})
