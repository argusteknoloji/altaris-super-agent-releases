...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'altaris-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    join('.altaris', 'wiki', 'schema.md'),
    join('.altaris', 'wiki', 'index.md'),
    join('.altaris', 'wiki', 'log.md'),
    join('.altaris', 'wiki', 'pages', 'architecture.md'),
  ])
