...
import type { EnvironmentMetadata } from '../../types/generated/events_mono/altaris/v1/altaris_internal_event.js'
 ...
 * - altaris.ai-proxied connectors — always official (from altaris.ai's list)
 * - Servers whose URL matches the official MCP registry — directory
 *   connectors added via `altaris mcp add`, not customer-specific config
 *
...
export type EventMetadata = {
  ...
  agentId?: string // ALTARIS_AGENT_ID (format: agentName@teamName) or subagent UUID
  parentSessionId?: string // ALTARIS_PARENT_SESSION_ID (team lead's session)
...
const buildEnvContext = memoize(async (): Promise<EnvContext> => {
  ...
  return {
    ...
    // the truth. ALTARIS_HOST_PLATFORM still overrides for container/remote.
    platformRaw: process.env.ALTARIS_HOST_PLATFORM || process.platform,
    arch: env.arch,
    ...
    isClaudeCodeRemote: isEnvTruthy(process.env.ALTARIS_REMOTE),
    isLocalAgentMode: process.env.ALTARIS_ENTRYPOINT === 'local-agent',
    isConductor: env.isConductor(),
    ...(process.env.ALTARIS_REMOTE_ENVIRONMENT_TYPE && {
      remoteEnvironmentType: process.env.ALTARIS_REMOTE_ENVIRONMENT_TYPE,
    }),
    // Gated by feature flag to prevent leaking "coworkerType" string in external builds
    ...(feature('COWORKER_TYPE_TELEMETRY')
      ? process.env.ALTARIS_COWORKER_TYPE
        ? { coworkerType: process.env.ALTARIS_COWORKER_TYPE }
        : {}
      : {}),
    ...(process.env.ALTARIS_CONTAINER_ID && {
      claudeCodeContainerId: process.env.ALTARIS_CONTAINER_ID,
    }),
    ...(process.env.ALTARIS_REMOTE_SESSION_ID && {
      claudeCodeRemoteSessionId: process.env.ALTARIS_REMOTE_SESSION_ID,
    }),
    ...(process.env.ALTARIS_TAGS && {
      tags: process.env.ALTARIS_TAGS,
    }),
    isGithubAction: isEnvTruthy(process.env.GITHUB_ACTIONS),
    isClaudeCodeAction: isEnvTruthy(process.env.ALTARIS_ACTION),
    ...
    ...(isEnvTruthy(process.env.GITHUB_ACTIONS) && {
      ...
      githubActionRef: process.env.GITHUB_ACTION_PATH?.includes(
        ...
        ? process.env.GITHUB_ACTION_PATH.split('altaris-action/')[1]
        : undefined,
    }),
...
export async function getEventMetadata(
  ...
  const metadata: EventMetadata = {
    ...
    envContext,
    ...(process.env.ALTARIS_ENTRYPOINT && {
      entrypoint: process.env.ALTARIS_ENTRYPOINT,
    }),
...
export function to1PEventFormat(
  ...
  //   event_schemas/.../altaris/v1/altaris_internal_event.proto
  ...
  const env: EnvironmentMetadata = {
    ...
    is_altaris_remote: envContext.isClaudeCodeRemote,
    ...
    is_altaris_action: envContext.isClaudeCodeAction,
  ...
  if (envContext.claudeCodeContainerId) {
    env.altaris_container_id = envContext.claudeCodeContainerId
  }
  if (envContext.claudeCodeRemoteSessionId) {
    env.altaris_remote_session_id = envContext.claudeCodeRemoteSessionId
  }
  if (envContext.tags) {
