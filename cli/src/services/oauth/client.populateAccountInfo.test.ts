...
test('OAuth account info population does not refresh when anthropic.com auth is inactive', () => {
  expect(
...
test('OAuth account info population still refreshes active anthropic.com auth', () => {
  expect(
    shouldRefreshOAuthAccountInfo({
      hasCompleteAccountInfo: false,
