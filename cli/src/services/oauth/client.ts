// OAuth client for handling authentication flows with Altaris services
...
/**
 * Check if the user has anthropic.com authentication scope
...
export function buildAuthUrl({
  ...
  authUrl.searchParams.append('code', 'true') // this tells the login page to show Altaris Max upsell
...
export async function refreshOAuthToken(
  ...
  const requestBody = {
    ...
    client_id: getOauthConfig().CLIENT_ID,
    // Request specific scopes, defaulting to the full Altaris AI set. The
  ...
  try {
    ...
    // ALTARIS_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.ALTARIS_ACCOUNT_UUID
  const envUserEmail = process.env.ALTARIS_USER_EMAIL
  const envOrganizationUuid = process.env.ALTARIS_ORGANIZATION_UUID
