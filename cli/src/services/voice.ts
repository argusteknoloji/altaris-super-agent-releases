...
export async function checkRecordingAvailability(): Promise<RecordingAvailability> {
  ...
  if (isRunningOnHomespace() || isEnvTruthy(process.env.ALTARIS_REMOTE)) {
    return {
      available: false,
      reason:
        'Voice mode requires microphone access, but no audio device is available in this environment.\n\nTo use voice mode, run Altaris locally instead.',
    }
  }
  ...
  const wslNoAudioReason =
    'Voice mode could not access an audio device in WSL.\n\nWSL2 with WSLg (Windows 11) provides audio via PulseAudio — if you are on Windows 10 or WSL1, run Altaris in native Windows instead.'
