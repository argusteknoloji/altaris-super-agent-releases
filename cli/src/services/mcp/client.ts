...
import { isClaudeInChromeMCPServer } from '../../utils/altarisInChrome/common.js'

// Lazy: toolRendering.tsx pulls React/ink; only needed when Altaris-in-Chrome MCP server is connected
/* eslint-disable @typescript-eslint/no-require-imports */
const altarisInChromeToolRendering =
  (): typeof import('../../utils/altarisInChrome/toolRendering.js') =>
    require('../../utils/altarisInChrome/toolRendering.js')
...
function handleRemoteAuthFailure(
  ...
  const label: Record<typeof transportType, string> = {
    sse: 'SSE',
    http: 'HTTP',
    'claudeai-proxy': 'altaris.ai proxy',
  }
  logMCPDebug(
    name,
...
/**
 * Fetch wrapper for altaris.ai proxy connections. Attaches the OAuth bearer
 ...
 * stale token mass-401s every altaris.ai connector and sticks them all in the
...
export function createClaudeAiProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    const doRequest = async () => {
      ...
      if (!currentTokens) {
        throw new Error('No altaris.ai OAuth token available')
      }
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        const wsHeaders = {
          'User-Agent': getMCPUserAgent(),
          ...(serverRef.authToken && {
            'X-Altaris-Code-Ide-Authorization': serverRef.authToken,
          }),
        }
        ...
        logMCPDebug(
          name,
          `Initializing altaris.ai proxy transport for server ${serverRef.id}`,
        )

        const tokens = getClaudeAIOAuthTokens()
        if (!tokens) {
          throw new Error('No altaris.ai OAuth token found')
        }

        const oauthConfig = getOauthConfig()
        const proxyUrl = `${oauthConfig.MCP_PROXY_URL}${oauthConfig.MCP_PROXY_PATH.replace('{server_id}', serverRef.id)}`

        logMCPDebug(name, `Using altaris.ai proxy at ${proxyUrl}`)
        ...
        logMCPDebug(name, `altaris.ai proxy transport created successfully`)
      } else if (
        ...
        const { createChromeContext } = await import(
          '../../utils/altarisInChrome/mcpServer.js'
        )
        ...
        const finalCommand =
          process.env.ALTARIS_SHELL_PREFIX || serverRef.command
        const finalArgs = process.env.ALTARIS_SHELL_PREFIX
      ...
      const client = new Client(
        {
          // name stays 'altaris' for compatibility with MCP servers that
          // gate features on the upstream client identifier.
          name: 'altaris',
          title: 'Altaris',
          version: MACRO.VERSION ?? 'unknown',
          description: 'Altaris — coding-agent CLI for any LLM provider',
      ...
      try {
        ...
        if (serverRef.type === 'sse' && error instanceof Error) {
          ...
          logMCPDebug(
            name,
            `altaris.ai proxy connection failed after ${elapsed}ms: ${error.message}`,
          )
          logMCPError(name, error)
        ...
        .map((tool): Tool => {
          ...
          return {
            ...
            ...(isClaudeInChromeMCPServer(client.name) &&
              ...
              ? altarisInChromeToolRendering().getClaudeInChromeMCPToolOverrides(
                tool.name,
              )
              : {}),
            ...(feature('CHICAGO_MCP') &&
