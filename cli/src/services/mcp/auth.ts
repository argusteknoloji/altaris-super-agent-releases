 ...
 *
 * Per RFC 7009, public clients (like Altaris) should authenticate by including
 ...
 * `altaris mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'altaris mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
...
export async function performMCPOAuthFlow(
  ...
  //
  // Servers with `oauth.xaa` but ALTARIS_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set ALTARIS_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
    ...
    const authorizationCode = await new Promise<string>((resolve, reject) => {
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          res.writeHead(200, { 'Content-Type': 'text/html' })
          res.end(
            `<h1>Authentication Successful</h1><p>You can close this window. Return to Altaris.</p>`,
          )
          cleanup()
          resolveOnce(result.code)
        }
 ...
 * to the PKCE flow. See github.com/anthropics/altaris/issues/28258.
 */
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Altaris (${this.serverName})`,
   ...
   * but this is the shape ALTARIS.md flags under "Token/auth caching across
