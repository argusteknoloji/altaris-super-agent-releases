 ...
 * reply and emits notifications/altaris/channel/permission with
 ...
 * capabilities.experimental['altaris/channel/permission'].
 *
 * Kenneth's "would this let Altaris self-approve?": the approving party is
 * the human via the channel, not Altaris. But the trust boundary isn't the
...
export type ChannelPermissionCallbacks = {
   ...
   *  (notifications/altaris/channel/permission). Returns true if the ID
 ...
 * reply and emits notifications/altaris/channel/permission with {request_id,
...
>(
  ...
  return clients.filter(
    (c): c is T & { type: 'connected' } =>
      ...
      c.capabilities?.experimental?.['altaris/channel'] !== undefined &&
      c.capabilities?.experimental?.['altaris/channel/permission'] !== undefined,
  )
}

/**
 * Factory for the callbacks object. The pending Map is closed over — NOT
 * module-level (per src/ALTARIS.md), NOT in AppState (functions-in-state
 ...
 * (notifications/altaris/channel/permission) with the structured payload.
