 ...
 *   - sends `notifications/altaris/channel` notifications for inbound — this file
 *
 ...
 * Requires altaris.ai OAuth auth — API key users are blocked until
...
export const ChannelMessageNotificationSchema = lazySchema(() =>
  z.object({
    method: z.literal('notifications/altaris/channel'),
    params: z.object({
      content: z.string(),
 ...
 * this declare `capabilities.experimental['altaris/channel/permission']` and
 * emit this event INSTEAD of relaying "yes tbxkq" as text via
 * notifications/altaris/channel. Explicit opt-in per server — a channel that
...
export const CHANNEL_PERMISSION_METHOD =
  'notifications/altaris/channel/permission'
...
export const CHANNEL_PERMISSION_REQUEST_METHOD =
  'notifications/altaris/channel/permission_request'
 ...
 * altaris.ai auth; console orgs have no admin opt-in surface yet.
 *
 *   skip      Not a channel server, or managed org hasn't opted in, or
 *             not in session --channels. Connection stays up; handler
 *             not registered.
 *   register  Subscribe to notifications/altaris/channel.
 *
...
export function gateChannelServer(
  ...
  // Channel servers declare `experimental['altaris/channel']: {}` (MCP's
  // presence-signal idiom — same as `tools: {}`). Truthy covers `{}` and
  // `true`; absent/undefined/explicit-`false` all fail. Key matches the
  // notification method namespace (notifications/altaris/channel).
  if (!capabilities?.experimental?.['altaris/channel']) {
    return {
      action: 'skip',
      kind: 'capability',
      reason: 'server did not declare altaris/channel capability',
    }
  }
  ...
  if (!getClaudeAIOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires altaris.ai authentication (run /login)',
    }
  }
