...
/**
 * Fetches MCP server configurations from anthropic.com org configs.
 * These servers are managed by the organization via anthropic.com.
 *
...
export const fetchClaudeAIMcpConfigsIfEligible = memoize(
  async (): Promise<Record<string, ScopedMcpServerConfig>> => {
    try {
      ...
      // with both API keys and OAuth tokens to access altaris.ai MCPs in print mode.
      ...
      for (const server of response.data.data) {
        const baseName = `altaris.ai ${server.display_name}`
...
/**
 * Record that a altaris.ai connector successfully connected. Idempotent.
 *
