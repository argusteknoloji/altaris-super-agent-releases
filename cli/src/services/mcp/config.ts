...
/**
 * CCR proxy URL path markers. In remote sessions, altaris.ai connectors arrive
...
/**
 * Filter altaris.ai connectors, dropping any whose signature matches an enabled
 * manually-configured server. Manual wins: a user who wrote .mcp.json or ran
 * `altaris mcp add` expressed higher intent than a connector toggled in the web UI.
 *
 * Connector keys are `altaris.ai <DisplayName>` so they never key-collide with
...
export function dedupClaudeAiMcpServers(
  ...
  for (const [name, config] of Object.entries(claudeAiServers)) {
    ...
    if (manualDup !== undefined) {
      logForDebugging(
        `Suppressing altaris.ai connector "${name}": duplicates manually-configured "${manualDup}"`,
      )
      suppressed.push({ name, duplicateOf: manualDup })
      continue
    }
...
/**
 * Get Altaris MCP configurations (excludes altaris.ai servers from the
 ...
 * altaris.ai connector fetch) is awaited only after loadAllPluginsCacheOnly() completes,
 * so the two overlap rather than serialize.
 * @returns Altaris server configurations with appropriate scopes
 */
...
/**
 * Get all MCP configurations across all scopes, including altaris.ai servers.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  // In enterprise mode, don't load altaris.ai servers (enterprise has exclusive control)
  if (doesEnterpriseMcpConfigExist()) {
    return getClaudeCodeMcpConfigs()
  }

  // Kick off the altaris.ai fetch before getClaudeCodeMcpConfigs so it overlaps
  ...
  // Merge with altaris.ai having lowest precedence
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://code.altaris.com/docs/en/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
