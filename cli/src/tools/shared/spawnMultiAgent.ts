...
/**
 * Ensures a team file exists on disk. If it doesn't (e.g. when a non-Altaris
...
async function handleSpawnSplitPane(
  ...
  // Build the command to spawn Altaris with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces ALTARIS_* env vars)
  const teammateArgs = [
  ...
  // Includes CLAUDECODE, ALTARIS_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new Altaris instance.
