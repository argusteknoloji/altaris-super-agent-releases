...
export function createMcpAuthTool(
  ...
  return {
    ...
    async call(_input, context) {
      // altaris.ai connectors use a separate auth flow (handleClaudeAIAuth in
      ...
      if (config.type === 'claudeai-proxy') {
        return {
          data: {
            ...
            message: `This is a altaris.ai MCP connector. Ask the user to run /mcp and select "${serverName}" to authenticate.`,
          },
        }
      }
