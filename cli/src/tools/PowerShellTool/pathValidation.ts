...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .altaris is a dangerous directory
  // and internal editable paths live under ~/.altaris/ — matching the ordering in
  ...
  // have explicitly configured writable directories (e.g. /tmp/altaris/) —
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.altaris; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.altaris/settings.json (Altaris's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.altaris`, validate subsequent statements with
  // cwd='./.altaris'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, Altaris may only access files in the allowed working directories for this session: ${dirListStr}.`
