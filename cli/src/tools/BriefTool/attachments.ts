...
export async function resolveAttachments(
  ...
  // of the guard inside uploadBriefAttachment — ALTARIS.md: "helpers defined
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    // REPL does, at main.tsx init). ALTARIS_BRIEF_UPLOAD lets a host that
    // runs the CLI as a subprocess opt in — e.g. the cowork desktop bridge,
    // which already passes ALTARIS_OAUTH_TOKEN for auth.
    const shouldUpload =
      uploadCtx.replBridgeEnabled ||
      isEnvTruthy(process.env.ALTARIS_BRIEF_UPLOAD)
