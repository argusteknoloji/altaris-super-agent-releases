...
async function handleBroadcast(
  ...
  if (!teamName) {
    throw new Error(
      'Not in a team context. Create a team with Teammate spawnTeam first, or set ALTARIS_TEAM_NAME.',
    )
  }
  ...
  if (!senderName) {
    throw new Error(
      'Cannot broadcast: sender name is required. Set ALTARIS_AGENT_NAME.',
    )
  }
    ...
    async checkPermissions(input, _context) {
      if (feature('UDS_INBOX') && parseAddress(input.to).scheme === 'bridge') {
        return {
          ...
          message: `Send a message to Remote Control session ${input.to}? It arrives as a user prompt on the receiving Altaris (possibly another machine) via Anthropic's servers.`,
