...
const inputSchema = lazySchema(() =>
  z.strictObject({
    ...
    durable: semanticBoolean(z.boolean().optional()).describe(
      'true = persist to .altaris/scheduled_tasks.json and survive restarts. false (default) = in-memory only, dies when this Altaris session ends. Use true only when the user asks the task to survive across sessions.',
    ),
  }),
)
...
export const CronCreateTool = buildTool({
  ...
  mapToolResultToToolResultBlockParam(output, toolUseID) {
    const where = output.durable
      ? 'Persisted to .altaris/scheduled_tasks.json'
      : 'Session-only (not written to disk, dies when Altaris exits)'
    return {
