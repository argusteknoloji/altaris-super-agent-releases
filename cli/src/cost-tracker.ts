...
// other ALTARIS_* flags.
function shouldLogTokenUsageVerbose(): boolean {
  const v = (process.env.ALTARIS_LOG_TOKEN_USAGE ?? '').trim().toLowerCase()
  if (!v) return false
