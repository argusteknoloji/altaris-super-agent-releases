 ...
 * 1. SSETransport (SSE reads + POST writes) when ALTARIS_USE_CCR_V2 is set
 * 2. HybridTransport (WS reads + POST writes) when ALTARIS_POST_FOR_SESSION_INGRESS_V2 is set
...
export function getTransportForUrl(
  ...
  if (url.protocol === 'ws:' || url.protocol === 'wss:') {
    if (isEnvTruthy(process.env.ALTARIS_POST_FOR_SESSION_INGRESS_V2)) {
