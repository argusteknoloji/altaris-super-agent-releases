 ...
 * - Epoch management: reads worker_epoch from ALTARIS_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (ALTARIS_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * ALTARIS_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    ALTARIS_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.ALTARIS_WORKER_EPOCH
