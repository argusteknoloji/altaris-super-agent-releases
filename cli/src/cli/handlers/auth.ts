...
export async function authLogin({
  ...
  // Without it, --console selects Console; --claudeai (or no flag) selects altaris.ai.
  ...
  const envRefreshToken = process.env.ALTARIS_OAUTH_REFRESH_TOKEN
  if (envRefreshToken) {
    const envScopes = process.env.ALTARIS_OAUTH_SCOPES
    if (!envScopes) {
      process.stderr.write(
        'ALTARIS_OAUTH_SCOPES is required when using ALTARIS_OAUTH_REFRESH_TOKEN.\n' +
          'Set it to the space-separated scopes the refresh token was issued with\n' +
          '(e.g. "user:inference" or "user:profile user:inference user:sessions:altaris user:mcp_servers").\n',
      )
      process.exit(1)
    }
...
export async function authStatus(opts: {
  ...
  } else if (authTokenSource === 'altaris.ai') {
    authMethod = 'altaris.ai'
  ...
  if (opts.text) {
    ...
    if (!loggedIn) {
      process.stdout.write(
        'Not logged in. Run altaris auth login to authenticate.\n',
      )
    }
  } else {
