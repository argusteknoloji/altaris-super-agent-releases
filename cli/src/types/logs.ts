...
export type SerializedMessage = Message & {
  cwd: string
  userType: string
  entrypoint?: string // ALTARIS_ENTRYPOINT — distinguishes cli/sdk-ts/sdk-py/etc.
  sessionId: string
  timestamp: string
  version: string
  gitBranch?: string
 ...
 * `altaris ps` can show something more useful than the last user prompt
...
/**
 * Per-file attribution state tracking Altaris's character contributions.
 */
export type FileAttributionState = {
  contentHash: string // SHA-256 hash of file content
  claudeContribution: number // Characters written by Altaris
  mtime: number // File modification time
}

/**
 * Attribution snapshot message stored in session transcript.
 * Tracks character-level contributions by Altaris for commit attribution.
 */
