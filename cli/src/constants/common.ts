...
export function getLocalISODate(): string {
  ...
  if (process.env.ALTARIS_OVERRIDE_DATE) {
    return process.env.ALTARIS_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.ALTARIS_OVERRIDE_DATE
    ? new Date(process.env.ALTARIS_OVERRIDE_DATE)
    : new Date()
