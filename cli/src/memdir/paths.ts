 ...
 *   1. ALTARIS_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. ALTARIS_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no ALTARIS_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.ALTARIS_DISABLE_AUTO_MEMORY
 ...
 * Resolution order:
 *   1. ALTARIS_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.altaris (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.ALTARIS_REMOTE_MEMORY_DIR) {
    return process.env.ALTARIS_REMOTE_MEMORY_DIR
  }
 ...
 *
 * SECURITY: projectSettings (.altaris/settings.json committed to the repo) is
 ...
 * same repo share one auto-memory directory (anthropics/altaris#24382).
 */
