...
export default class App extends PureComponent<Props, State> {
  ...
  stdinMode: 'readable' | 'data' = process.env.ALTARIS_USE_DATA_STDIN === '1' || process.env.ALTARIS_USE_READABLE_STDIN === '0' ? 'data' : 'readable';
  ...
  override componentDidMount() {
    ...
    if (this.props.stdout.isTTY && !isEnvTruthy(process.env.ALTARIS_ACCESSIBILITY)) {
  ...
  handleSuspend = (): void => {
    ...
    // Emit suspend event for Altaris to handle. Mostly just has a notification
    ...
    const resumeHandler = () => {
      ...
      if (this.props.stdout.isTTY) {
        if (!isEnvTruthy(process.env.ALTARIS_ACCESSIBILITY)) {
          this.props.stdout.write(HIDE_CURSOR);
        }
      ...
      // Emit resume event for Altaris to handle
