...
export function useUnseenDivider(messageCount: number): {
  ...
  const onScrollAway = useCallback((handle: ScrollBoxHandle) => {
    ...
    //   at max (Sarah Deaton, #altaris-feedback 2026-03-15)
 ...
 * users think of as "a new message from Altaris" — not raw assistant entries
 ...
 * until Altaris's text response lands.
 */
 ...
 *
 * Fullscreen mode defaults on for ants (ALTARIS_NO_FLICKER=0 to opt out)
 * and off for external users (ALTARIS_NO_FLICKER=1 to opt in).
...
// conversation turn's prompt above the viewport so you know what Altaris was
