...
export function SpinnerWithVerb(props: Props): React.ReactNode {
  ...
  useMemo(() => isEnvTruthy(process.env.ALTARIS_BRIEF), []) : false;
...
function SpinnerWithVerbInner({
  ...
  const defaultColor: keyof Theme = 'altaris';
  ...
  const effectiveTip = contextTipsActive ? undefined : showClearTip && !nextTask ? 'Use /clear to start fresh when switching topics and free up context' : showBtwTip && !nextTask ? "Use /btw to ask a quick side question without interrupting Altaris's current work" : spinnerTip;
