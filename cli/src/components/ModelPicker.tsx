...
export type Props = {
   ...
   * project-scoped (written to the assistant's .altaris/settings.json via
   * install.ts) and should not leak to the user's global ~/.altaris/settings.
   */
...
export function ModelPicker(t0) {
  ...
  const t16 = headerText ?? "Switch between Altaris models. Applies to this session and future Altaris sessions. For other/previous model names, specify with --model.";
  let t17;
  if ($[42] !== t16) {
...
function EffortLevelIndicator(t0) {
  ...
  const t1 = effort ? "altaris" : "subtle";
  const t2 = effort ?? "low";
  let t3;
  if ($[0] !== t2) {
