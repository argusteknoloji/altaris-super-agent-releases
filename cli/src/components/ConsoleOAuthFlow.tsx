...
export function ConsoleOAuthFlow({
  ...
  const forcedMethodMessage = forceLoginMethod === 'claudeai' ? 'Login method pre-selected: Subscription Plan (Altaris Pro/Max)' : forceLoginMethod === 'console' ? 'Login method pre-selected: API Usage Billing (Anthropic Console)' : null;
  ...
  const [loginWithClaudeAi, setLoginWithClaudeAi] = useState(() => {
    // Use Altaris AI auth for setup-token mode to support user:inference scope
  ...
  const startOAuth = useCallback(async () => {
    try {
      ...
      if (mode === 'setup-token') {
        ...
        // Don't save to keychain - the token is displayed for manual use with ALTARIS_OAUTH_TOKEN
        setOAuthStatus({
          state: 'success',
        ...
        void sendNotification({
          message: 'Altaris login successful',
...
function OAuthStatusMessage({
  ...
  switch (oauthStatus.state) {
    case 'idle': {
      const promptText =
        startingMessage ||
        'Altaris can be used with your Altaris subscription or billed based on API usage through your Console account.'

      const loginOptions = [
        {
          label: (
            <Text>
              Altaris account with subscription ·{' '}
    ...
    case 'creating_api_key':
      return (
        <Box flexDirection="column" gap={1}>
          <Box>
            <Spinner />
            <Text>Creating API key for Altaris…</Text>
          </Box>
        </Box>
      )
