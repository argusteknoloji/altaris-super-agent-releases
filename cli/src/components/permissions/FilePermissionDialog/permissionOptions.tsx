...
/**
 * Check if a path is within the project's .altaris/ folder.
 * This is used to determine whether to show the special ".altaris folder" permission option.
 */
export function isInClaudeFolder(filePath: string): boolean {
  const absolutePath = expandPath(filePath);
  const claudeFolderPath = expandPath(`${getOriginalCwd()}/.altaris`);

  // Check if the path is within the project's .altaris folder
  const normalizedAbsolutePath = normalizeCaseForComparison(absolutePath);
  const normalizedClaudeFolderPath = normalizeCaseForComparison(claudeFolderPath);

  // Path must start with the .altaris folder path (and be inside it, not just the folder itself)
...
/**
 * Check if a path is within the global ~/.altaris/ folder.
 * This is used to determine whether to show the special ".altaris folder" permission option
 * for files in the user's home directory.
 */
export function isInGlobalClaudeFolder(filePath: string): boolean {
  const absolutePath = expandPath(filePath);
  const globalClaudeFolderPath = join(homedir(), '.altaris');
  const normalizedAbsolutePath = normalizeCaseForComparison(absolutePath);
...
export function getFilePermissionOptions({
  ...
  if (yesInputMode && onAcceptFeedbackChange) {
    options.push({
      ...
      label: 'Yes',
      value: 'yes',
      placeholder: 'and tell Altaris what to do next',
  ...
  // Check if this is a .altaris/ folder path (project or global)
  const inClaudeFolder = isInClaudeFolder(filePath);
  const inGlobalClaudeFolder = isInGlobalClaudeFolder(filePath);

  // Option 2: For .altaris/ folder, show special option instead of generic session option
  // Note: Session-level options are always shown since they only affect in-memory state,
  // not persisted settings. The allowManagedPermissionRulesOnly setting only restricts
  // persisted permission rules.
  if ((inClaudeFolder || inGlobalClaudeFolder) && operationType !== 'read') {
    options.push({
      label: 'Yes, and allow Altaris to edit its own settings for this session',
  ...
  if (noInputMode && onRejectFeedbackChange) {
    options.push({
      ...
      label: 'No',
      value: 'no',
      placeholder: 'and tell Altaris what to do differently',
