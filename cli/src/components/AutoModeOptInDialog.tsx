...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets Altaris handle permission prompts automatically — Altaris checks each tool call for risky actions and prompt injection before executing. Actions Altaris identifies as safe are executed, while actions Altaris identifies as risky are blocked and Altaris may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. Altaris can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
...
export function AutoModeOptInDialog(t0: Props) {
  ...
  if ($[4] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = <Box flexDirection="column" gap={1}><Text>{AUTO_MODE_DESCRIPTION}</Text><Link url="https://code.altaris.com/docs/en/security" /></Box>;
    $[4] = t3;
  } else {
    t3 = $[4];
  }
