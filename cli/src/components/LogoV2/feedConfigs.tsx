...
export function createWhatsNewFeed(releaseNotes: string[]): FeedConfig {
  ...
  return {
    title: "external" === 'ant' ? "Altaris Updates [internal-only: Latest CC commits]" : "Altaris Updates",
    lines,
...
export function createProjectOnboardingFeed(steps: Step[]): FeedConfig {
  ...
  const warningText = getCwd() === homedir() ? 'Note: You have launched altaris in your home directory. For the best experience, launch it in a project directory instead.' : undefined;
  if (warningText) {
    lines.push({
      text: warningText
    });
...
export function createGuestPassesFeed(): FeedConfig {
  ...
  const subtitle = reward ? `Share Altaris and earn ${formatCreditAmount(reward)} of extra usage` : 'Share Altaris with friends';
  return {
