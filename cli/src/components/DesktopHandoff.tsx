...
export function getDownloadUrl(): string {
  switch (process.platform) {
    case 'win32':
      return 'https://altaris.ai/api/desktop/win32/x64/exe/latest/redirect';
    default:
      return 'https://altaris.ai/api/desktop/darwin/universal/dmg/latest/redirect';
  }
}
...
export function DesktopHandoff(t0) {
    ...
    t2 = () => {
      const performHandoff = async function performHandoff() {
        ...
        if (installStatus.status === "not-installed") {
          setDownloadMessage("Altaris Desktop is not installed.");
          setState("prompt-download");
          return;
        }
        if (installStatus.status === "version-too-old") {
          setDownloadMessage(`Altaris Desktop needs to be updated (found v${installStatus.version}, need v1.1.2396+).`);
          setState("prompt-download");
          return;
        }
        setState("flushing");
        ...
        if (!result.success) {
          setError(result.error ?? "Failed to open Altaris Desktop");
          setState("error");
          return;
        }
        setState("success");
  ...
  if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
    t4 = {
      checking: "Checking for Altaris Desktop\u2026",
      flushing: "Saving session\u2026",
      opening: "Opening Altaris Desktop\u2026",
      success: "Opening in Altaris Desktop\u2026"
    };
    $[17] = t4;
  } else {
    t4 = $[17];
...
async function _temp2(onDone_0) {
  onDone_0("Session transferred to Altaris Desktop", {
    display: "system"
  });
