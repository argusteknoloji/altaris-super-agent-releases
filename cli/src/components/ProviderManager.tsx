...
function CodexOAuthSetup({
  ...
  return (
    <Box flexDirection="column" gap={1}>
      ...
      <Text>
        Sign in with your ChatGPT account in the browser. Altaris will store
...
export function ProviderManager({ mode, onDone }: Props): React.ReactNode {
  ...
  const initialIsGithubActive = isEnvTruthy(process.env.ALTARIS_USE_GITHUB)
  ...
  const refreshGithubProviderState = React.useCallback((): void => {
    ...
    const githubActive = isEnvTruthy(process.env.ALTARIS_USE_GITHUB)
    ...
    void (async () => {
      ...
      setIsGithubActive(isEnvTruthy(process.env.ALTARIS_USE_GITHUB))
      setIsGithubCredentialSourceResolved(true)
    })()
  }, [])
  ...
  function buildCodexOAuthActivationMessage(options: {
    ...
    if (options.warnings.length > 0) {
      return `${options.prefix}. Altaris switched to it for this session with warnings: ${options.warnings.join('; ')}.`
    }

    return `${options.prefix}. Altaris switched to it for this session.`
  }
  ...
  function activateGithubProvider(): string | null {
    const { error } = updateSettingsForSource('userSettings', {
      env: {
        ...
        ALTARIS_USE_OPENAI: undefined as any,
        ALTARIS_USE_GEMINI: undefined as any,
        ALTARIS_USE_BEDROCK: undefined as any,
        ALTARIS_USE_VERTEX: undefined as any,
        ALTARIS_USE_FOUNDRY: undefined as any,
      },
    })
    if (error) {
  ...
  function deleteGithubProvider(): string | null {
    ...
    const { error } = updateSettingsForSource('userSettings', {
      env: {
        ALTARIS_USE_GITHUB: undefined as any,
  ...
  function renderPresetSelection(): React.ReactNode {
    ...
    const options = [
      ...
      },
      {
        value: 'anthropic',
        label: 'Anthropic',
        description: 'Native Model Provider API (x-api-key auth)',
      },
      {
        value: 'atomic-chat',
        label: 'Atomic Chat',
