...
export function isCoordinatorMode(): boolean {
  if (feature('COORDINATOR_MODE')) {
    return isEnvTruthy(process.env.ALTARIS_COORDINATOR_MODE)
  }
  return false
}
...
export function matchSessionMode(
  ...
  if (sessionIsCoordinator) {
    process.env.ALTARIS_COORDINATOR_MODE = '1'
  } else {
    delete process.env.ALTARIS_COORDINATOR_MODE
  }
...
export function getCoordinatorUserContext(
  ...
  const workerTools = isEnvTruthy(process.env.ALTARIS_SIMPLE)
...
export function getCoordinatorSystemPrompt(): string {
  const workerCapabilities = isEnvTruthy(process.env.ALTARIS_SIMPLE)
  ...
  return `You are Altaris, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
2. Precise git operation: "Create a new branch from main called 'fix/session-expiry'. Cherry-pick only commit abc123 onto it. Push and create a draft PR targeting main. Add anthropics/altaris as reviewer. Report the PR URL."
