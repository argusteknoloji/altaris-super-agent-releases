...
export function Doctor(t0) {
  ...
  if ($[5] === Symbol.for("react.memo_cache_sentinel")) {
    const envVars = [{
      ...
      name: "ALTARIS_MAX_OUTPUT_TOKENS",
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        ...
        const projectAgentsDir = join(getOriginalCwd(), ".altaris", "agents");
        const {
          activeAgents,
          allAgents,
          failedFiles
        ...
        if (isPidBasedLockingEnabled()) {
          const locksDir = join(getXDGStateHome(), "altaris", "locks");
  ...
  if ($[11] !== onDone) {
    t7 = () => {
      onDone("Altaris diagnostics dismissed", {
        display: "system"
      });
    };
    $[11] = onDone;
