...
import { getMemoryFiles } from '../utils/altarismd.js';
...
// repin to bottom. Josh Rosen's workflow: Altaris emits long output → scroll
...
export type Props = {
  ...
  // Direct connect config for `altaris connect` mode (connects to a altaris server)
  directConnectConfig?: DirectConnectConfig;
  // SSH session for `altaris ssh` mode (local REPL, remote tools over ssh)
...
export function REPL({
  ...
  const disableVirtualScroll = useMemo(() => isEnvTruthy(process.env.ALTARIS_DISABLE_VIRTUAL_SCROLL), []);
  const disableMessageActions = feature('MESSAGE_ACTIONS') ?
    // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
    useMemo(() => isEnvTruthy(process.env.ALTARIS_DISABLE_MESSAGE_ACTIONS), []) : false;
  ...
  // useEffect-based state initialization on mount (per ALTARIS.md guidelines)
  ...
  // from ALTARIS_NO_FLICKER=0 (which is process-lifetime) — this is
  ...
  // Allow Altaris Browser Bridge MCP to send prompts through MCP notifications
  ...
  // This enables "immediate" commands (like /btw) to persist while Altaris is processing.
  ...
  const terminalTitle = sessionTitle ?? agentTitle ?? haikuTitle ?? 'Altaris';
  ...
  // Prevent macOS from sleeping while Altaris is working
  useEffect(() => {
  ...
  // Push status to the PID file for `altaris ps`. Fire-and-forget; ps falls
  ...
  // Direct connect hook - manages WebSocket to a altaris server for `altaris connect` mode
  ...
  // SSH session hook - manages ssh child process for `altaris ssh` mode.
  ...
  useEffect(() => {
    ...
    setMessages(prev => [...prev, createSystemMessage(`Worktree creation took ${secs}s. For large repos, set \`worktree.sparsePaths\` in .altaris/settings.json to check out only the directories you need — e.g. \`{"worktree": {"sparsePaths": ["src", "packages/foo"]}}\`.`, 'info')]);
  }, [setMessages]);
  ...
  const resume = useCallback(async (sessionId: UUID, log: LogOption, entrypoint: ResumeEntrypoint) => {
    ...
    try {
      ...
      // JSONL with the fork's sessionId, so `altaris -r {forkId}` also works.
  ...
  // Session-level dedup for nested_memory ALTARIS.md attachments.
  // readFileState is a 100-entry LRU; once it evicts a ALTARIS.md path,
  ...
  // This allows Altaris to edit files that were read in previous sessions
  ...
  const sandboxAskCallback: SandboxAskCallback = useCallback(async (hostPattern: NetworkHostPattern) => {
    ...
    return new Promise(resolveShouldAllowHost => {
      ...
      // remote user (e.g. on altaris.ai) can approve it too.
  ...
  const onQueryImpl = useCallback(async (messagesIncludingNewMessages: MessageType[], newMessages: MessageType[], abortController: AbortController, shouldQuery: boolean, additionalAllowedTools: string[], mainLoopModelParam: string, effort?: EffortValue) => {
    ...
    // Mark onboarding as complete when any user message is sent to Altaris
    ...
    // title silently fell through to the "Altaris" default.
  ...
  const onSubmit = useCallback(async (input: string, helpers: PromptInputHelpers, speculationAccept?: {
    ...
    // even while Altaris is processing. Commands opt-in via `immediate: true`.
    ...
    // controls treatment: "dialog" (blocking), "hint" (notification), "off".
    {
      ...
      const idleThresholdMin = Number(process.env.ALTARIS_IDLE_THRESHOLD_MINUTES ?? 75);
      const tokenThreshold = Number(process.env.ALTARIS_IDLE_TOKEN_THRESHOLD ?? 100_000);
  ...
  // Handlers for auto-run /issue or /good-altaris (defined after onSubmit)
  ...
  async function onInit() {
    ...
    // Populate readFileState with ALTARIS.md files at startup
    const memoryFiles = await getMemoryFiles();
    if (memoryFiles.length > 0) {
      const fileList = memoryFiles.map(f => `  [${f.type}] ${f.path} (${f.content.length} chars)${f.parent ? ` (included by ${f.parent})` : ''}`).join('\n');
      logForDebugging(`Loaded ${memoryFiles.length} ALTARIS.md/rules files:\n${fileList}`);
    } else {
      logForDebugging('No ALTARIS.md/rules files found');
    }
  ...
  // for remote access via altaris.ai. No-op in external builds or when not enabled.
  const {
    sendBridgeResult
  ...
  // That write storm is the primary trigger for ~/.altaris.json corruption
  // (GH #3117).
  ...
  // Show notification when Altaris is done responding and user is idle
  useEffect(() => {
    // Don't set up notification if Altaris is busy
    if (isLoading) return;
    ...
    const timer = setTimeout((lastQueryCompletionTime, isLoading, toolJSX, focusedInputDialogRef, terminal) => {
      ...
      if (lastUserInteraction > lastQueryCompletionTime) {
        // User has interacted since Altaris finished - they're not idle, don't notify
        return;
      }
      ...
      if (!isLoading && !toolJSX &&
        ...
        void sendNotification({
          message: 'Altaris is waiting for your input',
  ...
  useEffect(() => {
    ...
    const idleThresholdMs = Number(process.env.ALTARIS_IDLE_THRESHOLD_MINUTES ?? 75) * 60_000;
  ...
  // Scheduled tasks from .altaris/scheduled_tasks.json (CronCreate/Delete/List)
  ...
  useEffect(() => {
    const handleSuspend = () => {
      ...
      process.stdout.write(`\nAltaris has been suspended. Run \`fg\` to bring Altaris back.\nNote: ctrl + z now suspends Altaris, ctrl + _ undoes input.\n`);
    };
    const handleResume = () => {
    ...
    <MCPConnectionManager key={remountKey} dynamicMcpConfig={dynamicMcpConfig} isStrictMcpConfig={strictMcpConfig}>
      <FullscreenLayout scrollRef={scrollRef} overlay={toolPermissionOverlay} bottomFloat={isBuddyEnabled() && companionVisible && !companionNarrow ? <CompanionFloatingBubble /> : undefined} modal={centeredModal} modalScrollRef={modalScrollRef} dividerYRef={dividerYRef} hidePill={!!viewedAgentTask} hideSticky={!!viewedTeammateTask} newMessageCount={unseenDivider?.count ?? 0} onPillClick={() => {
        ...
        {/* Hide the processing placeholder while a modal is showing —
                  ...
                  the user sees their input echoed while Altaris processes. */}
        ...
        <Box flexDirection="column" flexGrow={1}>
          ...
          {!toolJSX?.shouldHidePromptInput && !focusedInputDialog && !isExiting && !disabled && !cursor && !isShuttingDown() && <>
            ...
            {postCompactSurvey.state !== 'closed' ? <FeedbackSurvey state={postCompactSurvey.state} lastResponse={postCompactSurvey.lastResponse} handleSelect={postCompactSurvey.handleSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={handleSurveyRequestFeedback} /> : memorySurvey.state !== 'closed' ? <FeedbackSurvey state={memorySurvey.state} lastResponse={memorySurvey.lastResponse} handleSelect={memorySurvey.handleSelect} handleTranscriptSelect={memorySurvey.handleTranscriptSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={handleSurveyRequestFeedback} message="How well did Altaris use its memory? (optional)" /> : <FeedbackSurvey state={feedbackSurvey.state} lastResponse={feedbackSurvey.lastResponse} handleSelect={feedbackSurvey.handleSelect} handleTranscriptSelect={feedbackSurvey.handleTranscriptSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={didAutoRunIssueRef.current ? undefined : handleSurveyRequestFeedback} />}
